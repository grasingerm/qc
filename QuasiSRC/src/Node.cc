...
namespace quasicontinuum {
  ...
  namespace
  {
  	...
  	void clusterAddSiteLocal(struct node_t  *P_node, int  l[3])
  	{
		  ...
		  if(P_node->site_cluster.alloc == 0){
		    ...
		    if((P_node->site_cluster.sites =
		    	(int (*)[3]) malloc(SITE_ALLOC_CHUNK*sizeof(int[3]))) 
		    	== NULL)
		    {		      
		      PERROR_MALLOC();
		      ERROR("malloc()");
		   ...
		   // add node itself
		    ...
		    {		    
		      ...
		      for (i_site=0; i_site < shell->number_sites; i_site++) 
		      {
		      	...
		      	if (Lattice::getInstance()->isSiteInsideLattice(P_lattice,
		      		l, iQuasi) 
		      		== RETURN_FAILURE) 
		      		continue;
  	...
  	{
  		const int 		iQuasi = 
  			((struct BuildNodalClustersData_t *)arg)->iQuasi;
		  struct node_list_t          *P_node_list=
		  ...
		  BuildNodalClustersProcessingLocal(iQuasi,
		  	P_node_list, 
		  	P_lattice, 
		  	cluster_size,
		  	number_node_start, 
  	...
  	void RemoveOverlapsLocal(struct node_t *P_node_i, struct node_t *P_node_j)
  	{
		  ...
		  i_sites =
		  	(int (*)[3]) alloca(P_node_i->site_cluster.number_sites*
		  			sizeof(int[3]));
		  ...
		  j_sites = 
		  	(int (*)[3]) alloca(P_node_j->site_cluster.number_sites*
		  		sizeof(int[3]));
		  ...
		  mask_i =
		  	(int *) alloca(P_node_i->site_cluster.number_sites*sizeof(int));

		  memset(mask_i, '\0', P_node_i->site_cluster.number_sites*sizeof(int));

		  mask_j = 
		  	(int *) alloca(P_node_j->site_cluster.number_sites*sizeof(int));
		    ...
		    for (j_site=0; j_site < P_node_j->site_cluster.number_sites; j_site++)
		      if (( P_node_i->site_cluster.sites[i_site][0] 
		      	...
		      	&&( P_node_i->site_cluster.sites[i_site][2] 
		      	 == P_node_j->site_cluster.sites[j_site][2])) 
		      { /* same site found */
				 ...
				 const int 							 node_end)
  	{
		  ...
		  for (i_node=node_start; i_node <= node_end; i_node++)
		  {
		    ...
		    r_cluster_i = latticeC->getShellRadius(P_lattice, 
		    	P_shell->site[0], iQuasi);
		    ...
		    for (j_node=0; j_node < P_node_list->number_nodes; j_node++) 
		    {
		      ...
		      r_cluster_j = latticeC->getShellRadius(P_lattice, 
		      	P_shell->site[0], iQuasi);
  	...
  	void * ResolveOverlapsWorker(void * arg)
  	{
		  ...
		  while(1) 
		  {
		    ...
		    // do the work 
		    ResolveOverlapsProcessingLocal(iQuasi, 
		    		P_node_list,
		    		P_lattice,
		    		cluster_size,
		    		node_start,
  	...
  	double ComputeElementClusterRadiusLocal(const struct node_t *P_node)
  	{
		  ...
		  {		    
		    const struct element_t *P_element = 
		    	P_node->element_list.elements[i_elem];
		    int j_node;
		  ...
		  {		 
		    ...
		    for (i_site=0; i_site < P_shell->number_sites; i_site++) 
		    {
		      ...
		      for (i_elem=0; i_elem < P_node->element_list.number_elements; i_elem++) 
		      {
		      	...
		      	sum += Shape::getInstance()->computeSiteShapeFunction(P_node, 
		      				P_nodes[0], P_nodes[1], P_nodes[2], l, P_lattice, iQuasi);
				...
				struct lattice_t *P_lattice)
  	{
		  ...
		  for (i_site = 0; 
		       i_site < P_cluster_node->site_cluster.number_sites; 
		       i_site++) 
		  {
		    struct node_t    *P_nodes[3];
		    struct element_t *P_element;
  	...
  	void * ComputeClusterNodalWeightsLampedWorker(void * arg)
  	{
		  ...
		  {		    
		    ...
		    {		      
		      ...
		      // add self
		      cluster_sum = ComputeClusterShapeSumSelfLocal(iQuasi, 
		      		P_node_i, P_lattice);
		      ...
		      for(j_node=0; j_node < P_node_i->node_list.number_nodes; j_node++)
		      {
		      	...
		      	cluster_sum += 
		      	  ComputeClusterShapeSumOtherLocal(iQuasi, 
		      	  	P_node_j, P_node_i, P_lattice);
			    }
		      
		      // compute exact sum and weight
		      P_node_i->weight = 
		      	ComputeShapeFunctionSumLocal(iQuasi, 
		      		P_node_i, P_lattice)/cluster_sum;
		    }
		  }
		     ...
		     const int 									iQuasi)
  	{
		  ...
		  for (i_node = 0; i_node < P_node_list->number_nodes; i_node++) 
		  {
		    ...
		    if (P_node->weight < 1.0) 
		    {
		      ...
		      for (i_elem = 0; 
			   			 i_elem < P_node->element_list.number_elements; 
			   			 i_elem++) 
		      {
		      	struct element_t *P_element = 
		      	 			P_node->element_list.elements[i_elem];
		      	...
		      	// against the element
		      	for (i_site = 0; i_site < P_node->site_cluster.number_sites;
		      			 i_site++)
		      		if (P_element == 
		      			  Element::getInstance()->locateSiteElement(NULL, 
		      			  P_node->site_cluster.sites[i_site],P_lattice, iQuasi)) 
		      			i_hit++;
	...
	namespace
	{
		...
		//
		void getShareDuplicateNodesLocal(const int my_id,
			  const int number_threads,
			  const int number_data,
			  int *number_data_thread,
			  int *number_data_start1,
			  int *number_data_end1,
		...
		//
		void checkForDuplicateNodesProcessingLocal(const struct node_list_t *P_node_list,
			int start_node,
			int end_node)
		{
		  int i_node;
		    ...
		    for(j_node=i_node+1; j_node < P_node_list->number_nodes; j_node++)
		    {
			  			...
			  			(P_node_i->l[2] == P_node_j->l[2])) 
		      {
		      	...
		      	d_print("node %d: % e % e % e\n", i_node, 
		      		      P_node_i->initial_position[0],
		      		      P_node_i->initial_position[1],
		      		      P_node_i->initial_position[2]);
		      	d_print("node %d: % e % e % e\n", j_node, 
		      		      P_node_j->initial_position[0],
		      		      P_node_j->initial_position[1],
		      		      P_node_j->initial_position[2]);
		      }
		    }
		  }
		...
		void * checkForDuplicateNodesWorker(void * arg)
		{
		  ...
		  getShareDuplicateNodesLocal(my_id, 
					    number_threads, 
		  ...
		  checkForDuplicateNodesProcessingLocal(P_node_list, 
						       number_start1,
						       number_end1);
          ...
          enum mt_version_t         mt_version)  
  {
    ...
    switch(mt_version)
    {
      ...
      case MULTI_THREADED:
        ...
        {
          thread_bbox = (struct boundingbox_t *) 
                malloc(number_threads*sizeof(struct boundingbox_t));
        ...
        memcpy( P_boundingbox, &(thread_bbox[0]), 
            sizeof(struct boundingbox_t));
  ...
  void Node::cleanNodes(struct all_node_list_t  *P_node_list)  
  {
    ...
    for(j_node=0; 
        j_node < P_node_list->node_list.number_inactive_nodes; 
        j_node++)
    ...
    for(j_node=0; j_node < P_node_list->new_node_list.number_nodes; j_node++)
    {
      ...
      memcpy(P_node->position, P_new_node->position, 
       sizeof(P_node->position));
      ...
      memcpy(P_node->velocity, P_new_node->velocity, 
       sizeof(P_node->velocity));
  ...
  //	makeActiveNode()
  //
  void Node::makeActiveNode(struct node_list_t *P_node_list,
			     const int           i_node)
	{
	  static int init=0;
	  int j_node;
	  ...
	  if(init==0)
	  {
	    ...
	    if(node_bucket==0) node_bucket=1;
	  ...
	  if( P_node_list->number_inactive_nodes > 0 )
	  {
	    ...
	    if( P_node_list->alloc_space < i_node+1 )
	    {
	    	if( P_node_list->alloc_space == 0 )
	    		...
	    		P_node_list->nodes = 
	    	  (struct node_t **) realloc(P_node_list->nodes, (P_node_list->alloc_space+1)*sizeof(struct node_t *));

    	  #if defined(__QC_SGI)
    	    change_memory_placement(P_node_list->nodes, 
			      P_node_list->alloc_space + 1, 0);
    	  #endif /* sgi */
	  ...
	  if( P_node_list->alloc_space < i_node+1 )
	  {
	    ...
	    if( P_node_list->alloc_space == 0 )
	      ...
	      P_node_list->nodes = 
	    		(struct node_t **) realloc(P_node_list->nodes, 
	    ...
	    #if defined(__QC_SGI)
	      memset(P_node_list->nodes[i_node], 0, 
	      	node_bucket*sizeof(struct node_t));
	    #endif /* sgi */
	    ...
	    for(j_node=1; j_node < node_bucket; j_node++)
	    {
	      ...
	      pthread_mutex_init(&P_node_list->nodes[i_node+j_node]->site_cluster.lock,NULL);
	    }
	  }
  ...
  //	makeInactiveNode()
  //
  void Node::makeInactiveNode(struct node_list_t *P_node_list,
			       const int           i_node)
	{
	  ...
	  if( P_node_list->alloc_inactive_space < i_node+1 )
	  {
	      ...
	      P_node_list->inactive_nodes = 
	    		(struct node_t **) realloc(P_node_list->inactive_nodes,
  ...
  //	createNodalClusters()
  //
  void Node::createNodalClusters(int 	iQuasi,
  				struct node_list_t     *P_node_list,
		      struct lattice_t *P_lattice,
		      int              *cluster_size,
		      enum mt_version_t       mt_version)
	{
  	// create clusters
  	BuildNodalClusters(iQuasi, P_node_list, 
  		P_lattice, cluster_size, mt_version);
  ...
  int Node::nodalClusterWeightTest(const int iQuasi,
		    	...
		    	struct lattice_t      *P_lattice)
	{
	  ...
	  d_print("exact sum = % e approx sum = % e rel. error = % e\n", 
		 exact_sum, approx_sum, rel_error);

	  if(rel_error >= 1.0e-6) abort();
  ...
  void Node::computeNodalClusterSitesAndWeights(int iQuasi,
			...
			enum mt_version_t      mt_version)
	{
	  ...
	  if ((cluster_size = 
	  	(int *) malloc(P_node_list->number_nodes*sizeof(int))) 
	  	== NULL)
	  {
	    ERROR("malloc()");
	    exit(EXIT_FAILURE);
	  	...
	  	== NULL)
	  {
		...
		#if defined(__QC_SGI)
	  	change_memory_placement(cluster_size, 
	  		P_node_list->number_nodes*sizeof(int),0);
	  	change_memory_placement(cluster_incr, 
	  		P_node_list->number_nodes*sizeof(int),0);
		#endif /* sgi */
  ...
  //
  void Node::addNewNode(const int               l[3],
       ...
       const int 							 iQuasi)  
  {
	  ...
	  double atomicMass = Quasicontinua::getInstance()->getQuasi(iQuasi).getAtomicMass();
	  double boltzmanConstant = PairPotentials::getInstance()->getBoltzmanConstant();
	  ...
	  for(i_node=0; i_node < P_node_list->new_node_list.number_nodes; i_node++)
	  	if (latticeC->compareSites(l, 
	  			P_node_list->new_node_list.nodes[i_node]->l) 
	  			== RETURN_SUCCESS) 
	  	{
	  ...
	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
		   initial_position[0]), &(X[0]), sizeof(X));
	 
	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
	    position[0]), &(x[0]), sizeof(x));

	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
	     initial_frequency), &(W), sizeof(W));

	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
	     frequency), &(w), sizeof(w)); 

	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
	     initial_temperature), &(T), sizeof(T));

	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
	      temperature), &(t), sizeof(t));    

	  memcpy(&(P_node_list->new_node_list.nodes[number_new_nodes]->
	     initial_tau), &(Tau), sizeof(Tau));
  ...
  //	BuildNodalClusters()
  //
  void Node::BuildNodalClusters(int iQuasi,
  				struct node_list_t     *P_node_list,
		      struct lattice_t *P_lattice,
		      int              *cluster_size,
		      enum mt_version_t       mt_version)
	{
	  switch(mt_version)
	  {
	  	case MULTI_THREADED:
	    {
	      ...
	      thread_monitor(BuildNodalClustersWorker, (void *) &data, get_max_number_threads());
	    }
	   	break;
  ...
  //	ResolveOverlaps()
  //
  void Node::ResolveOverlaps(int 							iQuasi,
  		struct node_list_t     *P_node_list,
		  struct lattice_t *P_lattice,
		  int              *cluster_size,
		 enum mt_version_t       mt_version)
	{
	  ...
	  {
	  	case SINGLE_THREADED:
	  	  ResolveOverlapsProcessingLocal(iQuasi, P_node_list, 
	  	  	P_lattice, cluster_size, 0, P_node_list->number_nodes -1);
	    break;
  ...
  void Node::CheckClusters(const struct node_list_t *P_node_list,
	  ...
	  for (l[0] = P_lattice->l_start[0]; l[0] <= P_lattice->l_end[0]; l[0]++)
	    for (l[1] = P_lattice->l_start[1]; l[1] <= P_lattice->l_end[1]; l[1]++)
	      for (l[2] = P_lattice->l_start[2]; l[2] <= P_lattice->l_end[2]; l[2]++) 
	      {
	      	...
	      	Lattice::getInstance()->getSiteInitialPosition(X, l, 
	      			P_lattice, iQuasi);
			...
			struct lattice_t   *P_lattice)
	{
	  ...
	  thread_monitor(ComputeClusterNodalWeightsLampedWorker, 
			 (void *) &data, get_max_number_threads());
        ...
        const double        atomistic_element_size)
  {
  	...
  	// check weight
  	if(P_node->weight > 1.0 + tolerance ||
  		 P_node->weight < 1.0 - tolerance)
  		return false;
  	...
  	for(iElement = 0; iElement < element_list.number_elements; iElement++)
  	{
  		...
  		if(MiscFunctions::getInstance()->tetraVolume(P_element->node[0]->position,

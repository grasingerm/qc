...
namespace quasicontinuum {
  ...
  //
  void Output::createOutputDirectory(int outputFlag,
    char * outputDirectory)
  {
    ...
    if(init == 0)
    {
      ...
      sprintf(&(dirname[0]), "%d-%d_%d:%d:%d", d_tm.tm_mon+1, d_tm.tm_mday, d_tm.tm_hour, d_tm.tm_min, d_tm.tm_sec);

      //
      //  now find whether to create directory at default place or at place given by quasi.ini
      //

      //
      //  local_flag = 0 :  default output directory assuming outputDirectory variable is 
      //                    already allocated
      ...
      {
        //
        //  for this case we need to create output directory at the address given by outputDirectory
        //
        ...
        {
          local_flag = 0;
          // printf("creating output folder at default location bcause outputDirectory is not allocated in Input::mainInput() while flag is 1.\n");
        }

        if(outputDirectory[0] == '\0')
        {
          local_flag = 0;
          // printf("creating output folder at default location because output_directory is empty in quasi.ini\n");
        }
      ...
      {
        // printf("output_flag is 0 thus creating output directory at default place\n");
      ...
      else
      {
        ...
        if(outputDirectory == NULL)
          ...
          outputDirectory = (char * ) realloc(outputDirectory, path_max * sizeof(char));
  ...
  {
    //
    //  check if d_outputDirectory is empty, if yes then call createOutputDirectory() 
    //
    ...
    if(RunData::getInstance()->d_timeOut == true)
    {
      ...
      d_print("******** here\n");
      sprintf(filename, "%s/time_data_%d", 
        d_outputDirectory,
        numThreads); 
  ...
  //
  void Output::performOutput(const int    iQuasi,
          unsigned int                    flags,
          std::vector<int> output_flag_vector)
  {
    // get iQuasi instance
...
void Output::PerformLatticeOutput(const int iQuasi,
            const char                      *dirname, 
            ...
            int                             iteration_number)
{
  ...
  if(cg_info_flag == CONVERGED_OUTPUT)
    sprintf(filename, "%s/%squasi_%i_load_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark, 
      LATTICE_FILENAME_SUFFIX);  
  
  if(cg_info_flag == ITERATION_COMBINED_OUTPUT)
    sprintf(filename, "%s/iteration_combined_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark,
  ...
  if(cg_info_flag == ITERATION_FREQUENCY_OUTPUT)
    sprintf(filename, "%s/iteration_frequency_combined_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark,
  ...
  if(cg_info_flag == ITERATION_POSITION_OUTPUT)
    sprintf(filename, "%s/iteration_position_combined_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark,
  ...
  if(cg_info_flag == CONVERGED_INTERMEDIATE_COMBINED)
    sprintf(filename, "%s/converged_intermediate_combined_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark,
  ...
  if(cg_info_flag == CONVERGED_INTERMEDIATE_FREQUENCY)
    sprintf(filename, "%s/converged_intermediate_frequency_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark,
  ...
  if(cg_info_flag == CONVERGED_INTERMEDIATE_POSITION)
    sprintf(filename, "%s/converged_intermediate_position_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      LATTICE_FILENAME_PREFIX,
      iQuasi+1,
      mark,
  ...
  for (l[0] = P_lattice->l_start[0]; l[0] <= P_lattice->l_end[0]; l[0]++)
    for (l[1] = P_lattice->l_start[1]; l[1] <= P_lattice->l_end[1]; l[1]++)
      for (l[2] = P_lattice->l_start[2]; l[2] <= P_lattice->l_end[2]; l[2]++) {
        ...
        if (latticeClass->isLatticeSite(l, P_lattice, iQuasi) == RETURN_FAILURE) continue;

        /**
          * find element the site is in
          */

        P_element = Element::getInstance()->locateSiteElement(NULL, l, P_lattice,
          iQuasi);
        ...
        fprintf(file, "% 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e %d %d\n",
          S[0], S[1], S[2], S[3], t, s[0], s[1], s[2], s[3], t, P_element->number, 0);
  }
...
// PerformNodeOutput()
//
void Output::PerformNodeOutput(const int iQuasi,
            const char                  *dirname, 
            ...
            std::vector<int> output_flag_vector)
{
  ...
  sprintf(filename,"%s/node_data_%s%s",
    dirname,
    filename_temp,
  ...
  if(electro_enable == 1)
  {
    if(quadMethod == 1)
    {
      std::vector< double > traceOfK = Quasicontinua::getInstance()->getQuasi(iQuasi).getTraceOfKData();
      ...
      fprintf(file, " %s N= %d, E= %d, F=%s, ET=%s\n", 
        ...
        P_element_list->number_elements,
      ...
      for (i_node = 0; i_node < P_node_list->number_nodes; i_node++)
        fprintf(file, "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e"
          "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e "
          "%d %4s %d\n", 
          ...
          ElectricField[i_node][0],
          ElectricField[i_node][1],
    ...
    else
    {
      ...
      for (i_node = 0; i_node < P_node_list->number_nodes; i_node++)
        fprintf(file, "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e"
          "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e "
          "%d %4s %d\n", 
          P_node_list->nodes[i_node]->initial_position[0]+shift[0],
          P_node_list->nodes[i_node]->initial_position[1]+shift[1],
          P_node_list->nodes[i_node]->initial_position[2]+shift[2],
  ...
  else
  {
    if(quadMethod == 1)
    {
      ...
      for (i_node = 0; i_node < P_node_list->number_nodes; i_node++)
      {
        fprintf(file, "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e"
          "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e "
          "%d %4s %d\n", 
          P_node_list->nodes[i_node]->initial_position[0]+shift[0],
          P_node_list->nodes[i_node]->initial_position[1]+shift[1],
          P_node_list->nodes[i_node]->initial_position[2]+shift[2],
    ...
    else
    {
      ...
      //   // WORKING
      //   // for (i_node = 0; i_node < P_node_list->number_nodes; i_node++)
      //   //   fprintf(file_1, "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e %d %d\n", 
      //   //     P_node_list->nodes[i_node]->initial_position[0]+shift[0],
      //   //     P_node_list->nodes[i_node]->initial_position[1]+shift[1],
      //   //     P_node_list->nodes[i_node]->initial_position[2]+shift[2],
      //   //     P_node_list->nodes[i_node]->initial_frequency,
      ...
      //   for (i_node = 0; i_node < P_node_list->number_nodes; i_node++)
      //     fprintf(file_1, "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e"
      //     "% 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e "
      //     "%d %d\n", 
      //     P_node_list->nodes[i_node]->initial_position[0]+shift[0],
      //     P_node_list->nodes[i_node]->initial_position[1]+shift[1],
...
// PerformInputFormat()
//
void Output::PerformInputFormat(const int  iQuasi,
        const char               *dirname,
        ...
        int                       iteration_number)
{
  ...
  {
    // d_print("ITERATION\n");
    sprintf(filename, "%s/iteration_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      INPUT_PREFIX, 
      iQuasi+1,
      mark,
  ...
  if(cg_info_flag == CONVERGED_INTERMEDIATE)
  {
    ...
    sprintf(filename, "%s/converged_intermediate_%squasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      INPUT_PREFIX, 
      iQuasi+1,
      mark,
  ...
  if (xdr_double(&xdrs, &P_lattice->a1[0]) == FALSE) XDR_ERR("xdr_double()");
  if (xdr_double(&xdrs, &P_lattice->a1[1]) == FALSE) XDR_ERR("xdr_double()");
  if (xdr_double(&xdrs, &P_lattice->a1[2]) == FALSE) XDR_ERR("xdr_double()");

  if (xdr_double(&xdrs, &P_lattice->a2[0]) == FALSE) XDR_ERR("xdr_double()");
  if (xdr_double(&xdrs, &P_lattice->a2[1]) == FALSE) XDR_ERR("xdr_double()");
  if (xdr_double(&xdrs, &P_lattice->a2[2]) == FALSE) XDR_ERR("xdr_double()");
  
  if (xdr_double(&xdrs, &P_lattice->a3[0]) == FALSE) XDR_ERR("xdr_double()");
  if (xdr_double(&xdrs, &P_lattice->a3[1]) == FALSE) XDR_ERR("xdr_double()");
  if (xdr_double(&xdrs, &P_lattice->a3[2]) == FALSE) XDR_ERR("xdr_double()");

  if (xdr_int(&xdrs, &P_lattice->l_start[0]) == FALSE) XDR_ERR("xdr_int()");
  if (xdr_int(&xdrs, &P_lattice->l_start[1]) == FALSE) XDR_ERR("xdr_int()");
  if (xdr_int(&xdrs, &P_lattice->l_start[2]) == FALSE) XDR_ERR("xdr_int()");

  if (xdr_int(&xdrs, &P_lattice->l_end[0]) == FALSE) XDR_ERR("xdr_int()");
  if (xdr_int(&xdrs, &P_lattice->l_end[1]) == FALSE) XDR_ERR("xdr_int()");
  if (xdr_int(&xdrs, &P_lattice->l_end[2]) == FALSE) XDR_ERR("xdr_int()");
  ...
  for (i_node = 0; i_node < P_node_list->number_nodes; i_node++) {
    ...
    if (xdr_int(&xdrs, &(P_node->l[0])) == FALSE) XDR_ERR("xdr_int()");
    if (xdr_int(&xdrs, &(P_node->l[1])) == FALSE) XDR_ERR("xdr_int()");
    if (xdr_int(&xdrs, &(P_node->l[2])) == FALSE) XDR_ERR("xdr_int()");

    if (xdr_u_int(&xdrs, &(P_node->fix_mask)) == FALSE) XDR_ERR("xdr_int()");
...
//
void Output::PerformClusterOutput(const int  iQuasi,
          const char         *dirname, 
          struct node_list_t *P_node_list, 
          struct lattice_t   *P_lattice, 
          int                 mark,
          ...
          int                 iteration_number)
{
  ...
  fprintf(file, " %s = \"%s\"\n", "TITLE", "Atoms");
  fprintf(file, " %s = \"X\" \"Y\" \"Z\" \"W\" \"T\" \"xx\" \"yy\" \"zz\" \"ww\" \"tt\" "
  ...
  for (i_node = 0; i_node < P_node_list->number_nodes; i_node++) {
    ...
    for (i_site = 0; i_site < P_node->site_cluster.number_sites; i_site++) {
      ...
      fprintf(file, "% 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e %d %d %d\n",
...
// PerformIndentOutput()
//
void Output::PerformIndentOutput(const int    iQuasi,
        const char              *dirname,
        ...
        int                     iteration_number)
{
  ...
  if(cg_info_flag == ITERATION_OUTPUT)
  {
    sprintf(filename, "%s/iteration_%squasi_%i_load_number_%05d%s", 
      dirname, 
      INDENT_PREFIX, 
      iQuasi+1,
      mark,
  ...
  fprintf(file, "% 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e % 8.6e\n",
    ...
    P_indentor->position[2], P_indentor->force[0],
    P_indentor->force[1], P_indentor->force[2],
    P_indentor->energy.potential);
                ...
                int                 iteration_number)
{
  ...
  if(cg_info_flag == CONVERGED_OUTPUT)
  {
    sprintf(filename, "%s/%squasi_%i_load_number_%05d%s", 
      dirname, 
      CROSSNEIGHBORLIST_CLUSTER_PREFIX, 
      iQuasi+1,
      mark,
      CLUSTER_SUFFIX);
  }
                ...
                int                 iteration_number)
{
  ...
  if(PairPotentials::getInstance()->d_EAMFlag == true)
  {
    ...
    if(cg_info_flag == CONVERGED_OUTPUT)
    {
      sprintf(filename, "%s/%squasi_%i_load_number_%05d%s", 
        dirname, 
        CROSSNEIGHBORLIST_NEIGHBOR_PREFIX, 
        iQuasi+1,
        mark,
        CLUSTER_SUFFIX);
    }
...
//
void Output::PerformClusterForcesOutput(const int iQuasi,
            const char                  *dirname, 
            int                          mark,
            int                          cg_info_flag,
            int                          iteration_number)
{
   ...
   * fill in filename
   */
  sprintf(filename, "%s/cluster_forces_%i%s", dirname, iQuasi + 1, NODE_FILENAME_SUFFIX);
  ...
  if(cg_info_flag == ITERATION_OUTPUT)
  {
    sprintf(filename, "%s/iteration_cluster_site_forces_quasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      iQuasi+1,
      mark,
      iteration_number,
  ...
  if(cg_info_flag == CONVERGED_INTERMEDIATE)
  {  
    sprintf(filename, "%s/converged_intermediate_cluster_site_forces_quasi_%i_load_number_%05d_iteration_number_%05d%s", 
      dirname, 
      iQuasi+1,
      mark,
      iteration_number,
  ...
  const std::vector< std::vector< std::pair< std::vector<double>, double > > > iQuasi_clusterForceEnergy = ForceEnergyCalculation::getInstance()->getQuasiClusterSiteForceData(iQuasi);
  ...
  for(int b=0; b < iQuasi_clusterForceEnergy.size(); b++)
    for(int c=0; c < iQuasi_clusterForceEnergy[b].size(); c++)
      fprintf(file, "% 6.4e %6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e % 6.4e\n",
...
//
void Output::PerformRestartOutput(const int       iQuasi,
            const char                    *dirname,
            ...
            std::vector<int> output_flag_vector)
{
  ...
  // add the node_data to the filename
  sprintf(filename,"%s/restart_%s%s",
    dirname,
    filename_temp,
    RESTART_SUFFIX);
  ...
  //  testEnergyOutput()
  //
  void Output::testEnergyOutput(std::vector< std::vector< std::vector< std::pair< std::vector< double >, std::vector<double> > > > > *test_cluster_force_energy, 
    ...
    std::vector< std::vector< std::pair< std::vector<double>, double > > > *test_total_node_force_energy)
  {
    ...
    for(int iQuasi=0; iQuasi< numQuasi; iQuasi++)
    {
      ...
      // get all data needed
      struct node_list_t *P_node_list = &(iQuasicontinuum.getNodeList().node_list);
      ...
      if(Electrostatics::getInstance()->isElectrostaticEnable() == 1)
      {
        if(PairPotentials::getInstance()->d_EAMFlag == true)
        {
          ...
          fprintf(file, " %s = \"X\" \"Y\" \"Z\" \"W\" \"T\""
            ...
            " \"fw_entropy\""
            " \"fx_electrostatics\" \"fy_electrostatics\" \"fz_electrostatics\" \"fw__electrostatics\""
      ...
      //
      const std::vector< std::vector< cluster_site_data_t > > iQuasi_clusterData = CrossNeighborList::getInstance()->getQuasiClusterData(iQuasi);
           ...
           const struct node_list_t *P_node_list)
  {
    ...
    for (int i_node = 0; i_node < P_node_list->number_nodes; i_node++)
    {
      ...
      fprintf(file,
        " %d %d %d %6.4e %6.4e %6.4e %6.4e %6.4e %d %d\n",
        P_node_list->nodes[i_node]->l[0],
...
//
void Output::OutputDataFilename(char * filename,
  int              iQuasi,
  std::vector<int> flags)
{
  ...
  switch(minimization_method)
  {
    case 0:
    {
      ...
      {
        // inside cg iteration
        sprintf(filename,"cgmethod_0_load_%d_iteration_data_cg_iter_%d_iQuasi_%d",
          load_number,
          cg_iteration_number,
          iQuasi);
      ...
      {
        // converged cg
        sprintf(filename,"cgmethod_0_load_%d_converged_cg_quasi_%d",
          load_number,
          iQuasi);
      }
      else
    ...
    case 1:
    {
      ...
      if(cg_status == 0)
      {
        ...
        if(freq_or_position_min == 0)
        {
          ...
          sprintf(filename,
            "cgmethod_1_load_%d_iteration_data_cg_iter_%d_alternate_iter_%d_w_min_quasi_%d",
            load_number,
            cg_iteration_number,
            alternate_iteration_number,
            iQuasi);
        }
        else
        {
          ...
          sprintf(filename,
            "cgmethod_1_load_%d_iteration_data_cg_iter_%d_alternate_iter_%d_u_min_quasi_%d",
            load_number,
            cg_iteration_number,
            alternate_iteration_number,
            iQuasi);
      ...
      else
      {
        // converged final
         sprintf(filename,
           "cgmethod_1_load_%d_converged_cg_final_quasi_%d",
           load_number,
           iQuasi);
      }
    ...
    case 2:
    {
      ...
      {
        // converged cg
        sprintf(filename,"cgmethod_2_load_%d_converged_cg_quasi_%d",
          load_number,
          iQuasi);
      }
      else
    ...
    case 3:
    {
      ...
      {
        // converged cg
        sprintf(filename,"cgmethod_3_load_%d_converged_cg_quasi_%d",
          load_number,
          iQuasi);
      }
      else

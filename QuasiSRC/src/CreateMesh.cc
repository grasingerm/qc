...
namespace quasicontinuum {
  ...
  namespace 
  {
    ...
    //
    double CheckEnergyStrainLocal(struct element_t * P_element,
              const int i_load)
    {
      double center[3];
      double err_norm;
      ...
      err_norm = 
      Element::getInstance()->computeElementStrain(P_element, i_load);

      /* if (err_norm < 0.0) return(0.0); */
  
      radius = MiscFunctions::getInstance()->
              findTetraCenter(&(P_element->node[0]->initial_position[0]),
                &(P_element->node[1]->initial_position[0]),
                &(P_element->node[2]->initial_position[0]),
                &(P_element->node[3]->initial_position[0]),
                center);
    ...
    void * CheckEnergyWorker(void *arg)
    {
      ...
      double r_contact_zone = 
        ((struct CheckEnergyData_t *)arg)->r_contact_zone*VICINITY_RADIUS*
        VICINITY_RADIUS;

      const int i_load = 
        ((struct CheckEnergyData_t *)arg)->i_load;
      ...
      get_share(my_id, number_threads, 
        P_element_list->number_elements,  &number_elem_thread, 
        &number_elem_start, &number_elem_end);
  ...
  void CreateMesh::createElementsCentral(int    computeForce)
  {
    ...
    for(int iQuasi = 0; iQuasi<numQuasi; iQuasi++)
    {
      ...
      {
        r_cluster_cutoff = 
          PairPotentials::getInstance()->getCutoffRadiusClusterList(potentialNumber);
      }
      else
      {
    ...
    CrossNeighborList::getInstance()->computeCrossNeighborList(rebuild_neighbor_flag,
    ...
    if(computeForce == 0)
    {
      ...
      // 
      // in very first call to createElementsCentral(), copy the energy to initial_energy of 
      //  all node list data. 
      // 
            ...
            int                         restartFlag)
  {
    ...
    if(restartFlag == 0)
    {
      ...
      elementC->createElementsFromMeshData(P_element_list, P_node_list,
          P_lattice, &tetra_data_mesh);
    }
    ...
    // will not match)
    nodeC->computeNodalClusterSitesAndWeights(iQuasi, P_lattice, 
        P_node_list, r_cluster_cutoff, SINGLE_THREADED); // See note above
  ...
  int  CreateMesh::remeshCentral(int i_load)
  {
    ...
    for(int iQuasi = 0; iQuasi < numQuasi; iQuasi++)
    {
      ...
      struct indentor_t & indentor =
        quasicontinua->getQuasi(iQuasi).getIndentor();
  ...
  //
  int CreateMesh::remesh(int                 iQuasi,
                struct element_list_t        *P_element_list,
                struct all_node_list_t       *P_node_list,
                struct lattice_t             *P_lattice,
                struct indentor_t            *P_indentor,
                ...
                int                          i_load)
  {
    ...
    CheckEnergyRemesh(&score_lists, P_element_list, P_node_list, 
         P_indentor, i_load, iQuasi);
    ...
    {
      RemeshElementBisection(score_lists[i_elem].P_element, 
        P_node_list,
        P_lattice,
        iQuasi);
                     ...
                     const int               iQuasi)  
  {
    ...
    if( (FindBisectionSite(l, P_lattice, P_element, iQuasi)) 
        == RETURN_SUCCESS)
    {
    ...
    //  FindBisectionSite()
    //
    int CreateMesh::FindBisectionSite(int          l[3],
        struct lattice_t                      *P_lattice,
        ...
        const int                                   iQuasi)
    {
      ...
      temp_edge.length=
        miscC->getDistanceSqrBetweenPoints(P_node_0->initial_position, P_node_1->initial_position);
      ...
      temp_edge.length=
        miscC->getDistanceSqrBetweenPoints(P_node_0->initial_position, 
      ...
      temp_edge.length=
        miscC->getDistanceSqrBetweenPoints(P_node_1->initial_position, 
         P_node_2->initial_position);
      ...
      temp_edge.length=
        miscC->getDistanceSqrBetweenPoints(P_node_2->initial_position, 
         P_node_3->initial_position);
      ...
      center[0]=(edges[0].node[0]->initial_position[0]+
          edges[0].node[1]->initial_position[0])/2.0;
      center[1]=(edges[0].node[0]->initial_position[1]+
           edges[0].node[1]->initial_position[1])/2.0;
      center[2]=(edges[0].node[0]->initial_position[2]+
          edges[0].node[1]->initial_position[2])/2.0;
      ...
      FindCommonEdgeElements(&neigh_elements, edges[0].node[0],
              edges[0].node[1]);
      ...
      if (latticeC->isSiteInsideLattice(P_lattice, l_center, iQuasi)
        ...
        if (miscC->checkSiteInTetra(P_node_0->initial_position,
            P_node_1->initial_position, 
            P_node_2->initial_position,
            P_node_3->initial_position, 
            l_center, P_lattice, iQuasi) == INSIDE)
        {
          if ( (latticeC->compareSites(l_center, P_node_0->l) 
                == RETURN_FAILURE) 
            && (latticeC->compareSites(l_center, P_node_1->l) 
                == RETURN_FAILURE) 
            && (latticeC->compareSites(l_center, P_node_2->l) 
                == RETURN_FAILURE) 
            && (latticeC->compareSites(l_center, P_node_3->l) 
                == RETURN_FAILURE))
             stop_flag = 1;
        }
      ...
      for(i_elem=0, r_min=0.0; i_elem < neigh_elements.size(); i_elem++)
        ...
        {
          r=miscC->getDistanceSqrBetweenPoints(center, 
              neigh_elements[i_elem]->node[i_node]->initial_position);
          r_min = MAX(r_min, r);
        }
        ...
        for (i_elem = 0; i_elem < neigh_elements.size(); i_elem++) 
        {
           ...
           l_i, P_lattice, iQuasi) == INSIDE) 
          {
            ...
            /**
              ...
              */
            if (latticeC->compareSites(l_i, P_node_0->l) == RETURN_SUCCESS) continue;
            if (latticeC->compareSites(l_i, P_node_1->l) == RETURN_SUCCESS) continue;
            if (latticeC->compareSites(l_i, P_node_2->l) == RETURN_SUCCESS) continue;
            if (latticeC->compareSites(l_i, P_node_3->l) == RETURN_SUCCESS) continue;
      ...
      if (site_found) return(RETURN_SUCCESS);
    ...
    //
    void CreateMesh::FindCommonEdgeElements(std::vector<struct element_t *>
          *P_neigh_elements,
    ...
    //  CompareEdges()
    //
    bool CreateMesh::CompareEdges(const edge_t& lhs, 
            const edge_t& rhs)
    {
      return lhs.length > rhs.length;
    }
          ...
          int                iQuasi)
    {
      ...
      Indent::getInstance()->findIndentorContactRadius(&checkEnergyData.r_contact_zone, NULL, &P_node_list->node_list, 
      ...
      checkEnergyData.r_contact_zone = checkEnergyData.r_contact_zone*checkEnergyData.r_contact_zone;
      checkEnergyData.i_load         = i_load;

      thread_monitor( CheckEnergyWorker, 
        (void *) &checkEnergyData, get_max_number_threads());

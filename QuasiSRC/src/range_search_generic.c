...
static const char rcsid[] =
    "$Id: range_search_generic.c,v 1.4 2002/03/07 23:53:08 knap Exp $";
#endif /* !lint */
...
static void range_search_add_object(struct object_list_t *P_object_list,
                                    void *P_object) {
  ...
  if (P_object_list->alloc_space == 0) {
    ...
    P_object_list->objects = realloc(P_object_list->objects,
                                     P_object_len * P_object_list->alloc_space);
...
static struct tnode_t *node_create(struct BSP_tree_t *BSP_tree) {
...
static void insert_object(struct BSP_tree_t *BSP_tree, struct tnode_t *P_tnode,
                          void *object,
  ...
  if (P_child_tnode == NULL) {
    ...
    if (P_tnode->plane == X)
      P_child_tnode->plane = Y;
    if (P_tnode->plane == Y)
      P_child_tnode->plane = Z;
    if (P_tnode->plane == Z)
      P_child_tnode->plane = X;
...
BSP_tree_create(void *base, size_t n_elem, size_t width,
  ...
  for (i_object = 1; i_object < n_elem; i_object++) {
    ...
    insert_object(BSP_tree, BSP_tree->P_head, (void *)address, get_object_pos);
  }
...
void BSP_tree_destroy(struct BSP_tree_t *BSP_tree) {
...
static int is_inside_rectangle(const void *object,
  ...
  if (P_rectangle->point1.x <= P_point.x &&
      ...
      P_rectangle->point2.y >= P_point.y && P_rectangle->point2.z >= P_point.z)
    return (1);
...
static void _range_search(struct object_list_t *P_object_list,

...
namespace quasicontinuum {
  ...
  // Constructor.
  //
  Quasicontinuum::Quasicontinuum(const int id,
          int    ac,
				  char * av[]) : 
    ...
    d_indentor(INDENTOR_INITIALIZER),
    d_qc_options(QC_OPTIONS_INITIALIZER),
    d_force_flag(0),
    d_energy_flag(0),
    d_output_flag(0),
    d_mt_version(MULTI_THREADED),
    d_mass(0.0)
  {
    // 
    //  set id
    ...
    d_qc_options = Input::getInstance()->quasiInput(d_id,
             &d_lattice,
						 &d_node_list,
						 &d_element_list,
						 &d_indentor,
    ...
    {
      d_positionFixityOriginal.push_back(d_node_list.node_list.nodes[iNode]->fix_mask);
      d_frequencyFixityOriginal.push_back(d_node_list.node_list.nodes[iNode]->fix_w_mask);
    }
  ...
  //
  const lattice_t &
  Quasicontinuum::getLattice() const
  {
  ...
  //
  const element_list_t &
  Quasicontinuum::getElementList() const
  {
    
    //
    //
    //
    return d_element_list;

  }

  element_list_t &
  Quasicontinuum::getElementList()
  {
  ...
  // node list accessors
  //
  const all_node_list_t &
  Quasicontinuum::getNodeList() const
  {
  ...
  //
  // indentor data accessors
  //
  const indentor_t &
  Quasicontinuum::getIndentor() const
  {
  ...
  //
  const char *
  Quasicontinuum::getMaterialName() const
  {
    
    //
    //
    //
    return d_materialName;

  }

  char *
  Quasicontinuum::getMaterialName()
  {
  ...
  //
  const double 
  Quasicontinuum::getAtomicMass() const
  {
  ...
  //         0 - if restart is off
  //
  const int Quasicontinuum::isRestartOn() const
  {
    //
    // check d_qc_options
    //
  ...
  // return total energy.
  //
  const double Quasicontinuum::getEnergy()
  {
    return d_node_list.energy.total;
  }
  ...
  //  applyDeformation()
  //
  void Quasicontinuum::applyDeformation(const int    flag,
          ...
          for(int iIndex = 0; iIndex < 3; ++iIndex)
            for(int jIndex = 0; jIndex < 3; ++jIndex)
              P_node->position[iIndex] +=
                def[iIndex][jIndex] * (P_node->initial_position[jIndex] 
                + shiftVector[jIndex]);
  ...
  //
  int Quasicontinuum::getNumberNodes()
  {
    //
    // get number of nodes
    //
    const int num_nodes = d_node_list.node_list.number_nodes;
  ...
  //
  // get number of sites in node cluster
  //
  int Quasicontinuum::getNumberSites(int iNode)
  {
  ...
  Quasicontinuum::getClusterSiteLocationInNodeCluster(int iNode,
          int iSite)
  {
    //
    ...
    int *l_i = &(d_node_list.node_list.nodes[iNode]->site_cluster.sites[iSite][0]);

    //
    // get spatial coordinates
    //
    Lattice::getInstance()->getSiteCurrentPosition(siteLocation,
                  l_i,
                  &d_lattice,
                  d_id);
  ...
  //
  std::pair<std::vector<int>, std::vector<double> > 
  Quasicontinuum::getClusterSiteStateInNodeCluster(int iNode,
          int iSite)
  {
    //
    // site location variables
    ...
    // get spatial coordinates
    //
    Lattice::getInstance()->getSiteCurrentState(siteState,
                  l_i,
                  &d_lattice,
                  d_id);
  ...
  //
  std::vector<double>
  Quasicontinuum::getLatticeLocation(std::vector<int> site)
  {
    //
    // site location variables
    //
  ...
  //
  std::vector<double>
  Quasicontinuum::getLatticeState(std::vector<int> site)
  {
    //
    // site location variables
    //
  ...
  //
  void 
  Quasicontinuum::resetToCurrentPosition(const std::vector<std::vector<double> > currentPositions)
  {
    //
    // loop over all nodes
    //
  ...
  //
  void 
  Quasicontinuum::resetToCurrentState(const std::vector<std::vector<double> > currentStates)
  {
    //
    // loop over all nodes
    //
  ...
  //
  void Quasicontinuum::FindSiteInLatticeNearPoint(const std::vector<int> & 
              site_index_guess,
              std::vector<int> & return_site) const
  {
    ...
    if (voidC->isVoidEnable() == 1 && 
        voidC->findSiteInVoidCache(site_index_guess, d_id ) 
        == RETURN_SUCCESS)
    {
      ...
      for (int i = -1; i <= 1; ++i)
      {
        ...
        for (int j = -1; j <= 1; ++j)
        {
          ...
          for (int k = -1; k <= 1; ++k)
          {
            ...
            if (latticeC->isSiteInsideLattice(&d_lattice, 
                &test_site[0], d_id)
                == RETURN_FAILURE)
              continue;

            // check if test site is a lattice site
            if (latticeC->isLatticeSite(&test_site[0], 
                &d_lattice, d_id) 
                == RETURN_FAILURE) 
              continue;
  ...
  {
    std::vector<std::pair<std::vector<int>,std::vector<double> > > returnSitesAndState;
    ...
    std::vector<double> iShift = 
      Quasicontinua::getInstance()->getShift(d_id);
    ...
    else
    {
      ...
      if(breakVariable == 0)
      {
        ...
        d_print("Site: %d   %d  %d\n",
          ...
          iLatticeCoordinates[1],
          iLatticeCoordinates[2]);
    ...
    { 
      ...
      latticeC->getSiteCurrentState(siteState, 
               siteLattice, 
               &d_lattice,
               d_id);
    ...
    while(sitesAdded != 0)
    {
      ...
      // get new shell
      //
      P_shell = latticeC->getShell(shell_number,
        &d_lattice);
          ...
          ++siteCounter)
      {
        ...
        //
        siteLattice[0] = 
          jLatticeCoordinates[0] + P_shell->site[siteCounter][0];
        siteLattice[1] = 
          jLatticeCoordinates[1] + P_shell->site[siteCounter][1];
        siteLattice[2] = 
          jLatticeCoordinates[2] + P_shell->site[siteCounter][2];
  ...
  Quasicontinuum::getNeighborStateAroundPoint(
    const std::pair<std::vector<int>,std::vector<double> > siteLocation,
    const double cutoffRadius) 
  {
      ...
      std::vector<std::vector<double> > & vector_return_locations)
  {
    ...
    latticeC->getSiteCurrentPosition(&return_location[0], 
      &return_site[0], 
      &d_lattice, 
      d_id);
    ...
    if(MiscFunctions::getInstance()->IsPointInSphere(return_location,
        site_location,
    ...
    const std::vector<int> base_site {return_site[0], return_site[1], return_site[2]};
    ...
    while(sites_added == true)
    {
      ...
      // loop over all sites in shell
      for (int site_counter = 0; site_counter < P_shell->number_sites; ++site_counter)
      {
        // set site lattice coordinates
        for(int i_dof = 0; i_dof < 3; ++i_dof)
          return_site[i_dof] = base_site[i_dof] + P_shell->site[site_counter][i_dof];
  ...
  void Quasicontinuum::modifyInitialConfiguration()
  {
    ...
    d_node_list.initial_energy.total      = d_node_list.initial_energy.kinetic 
      + d_node_list.initial_energy.potential;
  ...
  void Quasicontinuum::setPositionFixity(const int fixity_flag)
  {
    switch(fixity_flag)
    {
      case 0:
      {
        ...
        if(d_positionFixityOriginal.size() != d_node_list.node_list.number_nodes)
        {
          d_print("Check d_positionFixityOriginal and function setPositionFixity()\n");
          exit(EXIT_FAILURE);
        }
  ...
  void Quasicontinuum::setFrequencyFixity(const int fixity_flag)
  {
    switch(fixity_flag)
    {
      case 0:
      {
        ...
        if(d_frequencyFixityOriginal.size() != d_node_list.node_list.number_nodes)
        {
          d_print("Check d_frequencyFixityOriginal and function setPositionFixity()\n");
          exit(EXIT_FAILURE);
        }

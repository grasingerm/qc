...
namespace quasicontinuum {
  ...
  namespace
  {
    ...
    //
    void indentorNodePotentialLocal(double                  *force,
            double                  *energy,
            const double             r,
    ...
    void * indentorNodesInteractionWorker(void *arg)
    {
      ...
      const int iQuasi =
        ((struct indentorNodesInteractionData_t *)arg)->iQuasi;
      ...
      get_share(my_id, number_threads, P_node_list->number_nodes, &number_node_thread, &number_node_start, &number_node_end);
  ...
  //  1 : enabled
  //
  int Indent::isIndentEnable()  
  {
    return d_enableNanoIndentation;
  }
  ...
  void Indent::indentorNodesInteraction(int iQuasi)
  {
    ...
    //  get data of iQuasi
    //
    Quasicontinuum iQuasicontinuum = Quasicontinua::getInstance()->getQuasi(iQuasi);
  ...
  //
  void Indent::findIndentorContactRadius(double   *P_R,
                    double                   *P_r,
                    ...
                    enum mt_version_t         mt_version)  
  {
    ...
    for (i_node = 0; i_node < P_node_list->number_nodes; i_node++) 
    {
      ...
      if (r < r_cut_sqr) 
      {
        ...
        if (P_R) 
        {
          r_ij[0] = P_node->initial_position[0] 
                    - P_indentor->initial_position[0];
          r_ij[1] = P_node->initial_position[1]
                   - P_indentor->initial_position[1];

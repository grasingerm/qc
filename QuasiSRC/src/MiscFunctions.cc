...
MiscFunctions *MiscFunctions::getInstance() {
...
void MiscFunctions::destroyInstance() {
...
//  tetraVolume()
//
double MiscFunctions::tetraVolume(const double a[3], const double b[3],
...
//  getUnitVector()
//
int MiscFunctions::getUnitVector(const double v[3], double u[3]) {
...
enum loc_t MiscFunctions::checkPointInTetra(const double v1[3],
  ...
  det = -v21[2] * v31[1] * v41[0] + v21[1] * v31[2] * v41[0] +
        v21[2] * v31[0] * v41[1] - v21[0] * v31[2] * v41[1] -
        v21[1] * v31[0] * v41[2] + v21[0] * v31[1] * v41[2];

  /*
   * compute r
   */

  r = (-p[2] * v31[1] * v41[0] + p[1] * v31[2] * v41[0] +
       p[2] * v31[0] * v41[1] - p[0] * v31[2] * v41[1] -
       p[1] * v31[0] * v41[2] + p[0] * v31[1] * v41[2]) /
      det;
  ...
  s = (p[2] * v21[1] * v41[0] - p[1] * v21[2] * v41[0] -
       p[2] * v21[0] * v41[1] + p[0] * v21[2] * v41[1] +
       p[1] * v21[0] * v41[2] - p[0] * v21[1] * v41[2]) /
      det;
  ...
  t = (-p[2] * v21[1] * v31[0] + p[1] * v21[2] * v31[0] +
       p[2] * v21[0] * v31[1] - p[0] * v21[2] * v31[1] -
       p[1] * v21[0] * v31[2] + p[0] * v21[1] * v31[2]) /
      det;
  ...
  if (r < -CHECK_ERROR || r > 1.0 + CHECK_ERROR)
    return (OUTSIDE);
  if (s < -CHECK_ERROR || s > 1.0 + CHECK_ERROR)
    return (OUTSIDE);
  if (t < -CHECK_ERROR || t > 1.0 + CHECK_ERROR)
    return (OUTSIDE);
  if (u < -CHECK_ERROR || u > 1.0 + CHECK_ERROR)
    return (OUTSIDE);
...
enum loc_t MiscFunctions::checkSiteInTetra(const double v1[3],
                                           const double v2[3],
                                           const double v3[3],
                                           const double v4[3], const int l[3],
...
//  computeDeterminant()
//
double MiscFunctions::computeDeterminant(const double a[3], const double b[3],
                                         const double c[3]) {
...
//  solveLinearThreeD()
//
int MiscFunctions::solveLinearThreeD(const double a[3], const double b[3],
                                     const double c[3], const double d[3],
                                     double x[3]) {
...
//
std::vector<double> MiscFunctions::crossProduct3x3(std::vector<double> a,
                                                   std::vector<double> b) {
...
//
double MiscFunctions::determinant3x3(std::vector<std::vector<double>> a) {
...
//
double MiscFunctions::L2Norm(std::vector<double> a) {
...
//
double MiscFunctions::dotProduct(const std::vector<double> &a,
...
//
double MiscFunctions::TriangleArea(std::vector<double> vertexA,
...
// calculate area of triangle
//
std::vector<double> MiscFunctions::TriangleCenter(std::vector<double> vertexA,
                                                  std::vector<double> vertexB,
                                                  std::vector<double> vertexC) {
...
//
std::vector<double> MiscFunctions::getSphereRayIntersection(
    std::vector<double> center, std::vector<double> nodeInSphere,
    std::vector<double> nodeOutsideSphere, double radiusSquared) {
  ...
  double C =
      MiscFunctions::dotProduct(nodeInSphere, nodeInSphere) - radiusSquared;
...
// scale vector
//
std::vector<double> MiscFunctions::vectorScale(std::vector<double> vectorA,
                                               const double scale) {
...
//
bool MiscFunctions::triangleSphereIntersection(
    const std::vector<double> &vertexA, const std::vector<double> &vertexB,
    const std::vector<double> &vertexC, const std::vector<double> &sphereCenter,
  ...
  std::vector<double> V =
      crossProduct3x3(vectorDifference(B, A), vectorDifference(C, A));
  ...
  std::vector<double> Q1 =
      vectorDifference(vectorScale(A, e1), vectorScale(AB, d1));
  std::vector<double> Q2 =
      vectorDifference(vectorScale(B, e2), vectorScale(BC, d2));
  std::vector<double> Q3 =
      vectorDifference(vectorScale(C, e3), vectorScale(CA, d3));
  ...
  bool sep5 = (dotProduct(Q1, Q1) > sphereRadiusSquared * e1 * e1) &
              (dotProduct(Q1, QC) > 0);
  bool sep6 = (dotProduct(Q2, Q2) > sphereRadiusSquared * e2 * e2) &
              (dotProduct(Q2, QA) > 0);
  bool sep7 = (dotProduct(Q3, Q3) > sphereRadiusSquared * e3 * e3) &
              (dotProduct(Q3, QB) > 0);
...
// triangle sphere intersection
//
bool MiscFunctions::edgeSphereIntersection(
    const std::vector<double> &vertexA, const std::vector<double> &vertexB,
    const std::vector<double> &sphereCenter, const double &sphereRadiusSquared,
  ...
  const double b = 2.0 * (dp[0] * (vertexA[0] - sphereCenter[0]) +
                          dp[1] * (vertexA[1] - sphereCenter[1]) +
                          dp[2] * (vertexA[2] - sphereCenter[2]));
  const double c =
      dotProduct(sphereCenter, sphereCenter) + dotProduct(vertexA, vertexA) -
      2.0 * dotProduct(sphereCenter, vertexA) - sphereRadiusSquared;

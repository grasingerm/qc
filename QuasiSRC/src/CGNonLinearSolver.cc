...
//
void *threadInitializeDirectionWorker(void *arg) {
  ...
  // get share of work
  //
  quasicontinuum::get_share(my_id, number_threads,
  ...
  // get handles
  //
  std::vector<double> &d_sOld = *(initializeDirection_argument->d_sOld);
  std::vector<double> &d_direction =
      *(initializeDirection_argument->d_direction);
  std::vector<double> &d_gradient = *(initializeDirection_argument->d_gradient);
  std::vector<double> &d_s = *(initializeDirection_argument->d_s);
#if 0 // defined(HAVE_MPI)
           std::vector<int> & d_unknownCountFlag = 
...
//
void *threadComputeDeltaDWorker(void *arg) {
  ...
  // get share of work
  //
  quasicontinuum::get_share(my_id, number_threads, computeDeltaD_argument->size,
                            &number_data_thread, &number_start, &number_end);

  //
  // get handle
  //
  std::vector<double> &d_gradient = *(computeDeltaD_argument->d_gradient);
  std::vector<double> &d_direction = *(computeDeltaD_argument->d_direction);
#if 0 // defined(HAVE_MPI)
         std::vector<int> & d_unknownCountFlag = 
...
//
void *threadComputeEtaWorker(void *arg) {
  ...
  // get share of work
  //
  quasicontinuum::get_share(my_id, number_threads, computeEta_argument->size,
                            &number_data_thread, &number_start, &number_end);

  //
  // get handle
  //
  std::vector<double> &d_gradient = *(computeEta_argument->d_gradient);
  std::vector<double> &d_direction = *(computeEta_argument->d_direction);
#if 0 // defined(HAVE_MPI)
         std::vector<int> & d_unknownCountFlag = 
...
//
void *threadComputeDeltasWorker(void *arg) {
  ...
  // get share of work
  //
  quasicontinuum::get_share(my_id, number_threads, computeDeltas_argument->size,
                            &number_data_thread, &number_start, &number_end);

  //
  // get handle
  //
  std::vector<double> &d_sOld = *(computeDeltas_argument->d_sOld);
  std::vector<double> &d_gradient = *(computeDeltas_argument->d_gradient);
  std::vector<double> &d_s = *(computeDeltas_argument->d_s);
#if 0 // defined(HAVE_MPI)
         std::vector<int> & d_unknownCountFlag = 
...
//
void *threadUpdateDirectionWorker(void *arg) {
  ...
  //
  quasicontinuum::get_share(my_id, number_threads,
                            updateDirection_argument->size, &number_data_thread,
                            &number_start, &number_end);

  //
  // get handle
  //
  std::vector<double> &d_direction = *(updateDirection_argument->d_direction);
  std::vector<double> &d_s = *(updateDirection_argument->d_s);
...
//
void *threadComputeResidualL2NormWorker(void *arg) {
...
//
void *threadUpdateSolutionWorker(void *arg) {
  ...
  //
  quasicontinuum::get_share(my_id, number_threads,
                            updateSolution_argument->size, &number_data_thread,
                            &number_start, &number_end);

  //
  // get handle
  //
  std::vector<double> &d_solution = *(updateSolution_argument->d_solution);
  const std::vector<double> &direction = *(updateSolution_argument->direction);
  const std::vector<int> &unknownCountFlag =
  ...
  for (int i = number_start; i <= number_end; ++i) {
    ...
    //
    d_solution[i] =
        (d_solution[i] + updateSolution_argument->alpha * direction[i]) *
        unknownCountFlag[i];
  }
  //
  //
...
// Constructor.
//
CGNonLinearSolver::CGNonLinearSolver(double tolerance, int maxNumberIterations,
                                     int debugLevel, double lineSearchTolerance,
                                     int lineSearchMaxIterations,
                                     double rebuild_cutoff, int minMethod,
                                     int minMethodMaxIterations,
                                     double minMethodTolerance)
    : NonLinearSolver(tolerance, maxNumberIterations, debugLevel),
      d_lineSearchTolerance(lineSearchTolerance),
      d_lineSearchMaxIterations(lineSearchMaxIterations),
      d_version(quasicontinuum::MULTI_THREADED), REBUILD(1), NO_REBUILD(0),
      d_accumulated_r(0.0), d_rebuild_cutoff(rebuild_cutoff),
      d_minMethod(minMethod), d_minMethodMaxIterations(minMethodMaxIterations),
...
//
void CGNonLinearSolver::initializeDirection() {
...
//
  ...
  case quasicontinuum::MULTI_THREADED: {
    ...
    initializeDirection_argument.d_unknownCountFlag = &(d_unknownCountFlag);
    ...
    //
    quasicontinuum::thread_monitor(
        threadInitializeDirectionWorker,
...
//
std::pair<double, double> CGNonLinearSolver::computeDeltaD() {
...
// Compute eta.
//
double CGNonLinearSolver::computeEta() {
...
//
void CGNonLinearSolver::computeDeltas() {
...
// Update direction.
//
void CGNonLinearSolver::updateDirection() {
...
//
double CGNonLinearSolver::computeResidualL2Norm() const {
...
//
  ...
  case quasicontinuum::MULTI_THREADED: {
    ...
    //
    quasicontinuum::thread_monitor(
        threadComputeResidualL2NormWorker,
...
//
int CGNonLinearSolver::computeTotalNumberUnknowns() const {
...
//
void CGNonLinearSolver::updateSolution(double alpha,
                                       const std::vector<double> &direction,
  ...
  case quasicontinuum::SINGLE_THREADED: {
    ...
    for (std::vector<double>::size_type i = 0; i < solutionSize; ++i)
      d_solution[i] =
          (d_solution[i] + alpha * direction[i]) * unknownCountFlag[i];
  }
...
CGNonLinearSolver::lineSearch(SolverFunction &function, double tolerance,
                              int maxNumberIterations, int debugLevel) {
  //
  // local data
  //
  ...
  // update unknowns
  //
  CGNonLinearSolver::updateSolution(sigma0, d_direction, function);
  ...
  for (int iter = 0; iter < maxNumberIterations; ++iter) {
    ...
    for (unsigned int iDirection = 0; iDirection < d_direction.size();
         ++iDirection)
    ...
    // update unknowns
    //
    CGNonLinearSolver::updateSolution(alpha, d_direction, function);
...
//
NonLinearSolver::ReturnValueType CGNonLinearSolver::solve(
    SolverFunction &function, double tolerance, int maxNumberIterations,
    int debugLevel, int output_flag, int loadNumber, int minMethod,
    int minMethodMaxIterations, double minMethodTolerance) {
#if 0 // defined(HAVE_MPI)
      //
      // get MPIController
      //
  ...
  case 0: {
    ...
    d_print(
        "CG Minimization: Minimizing free energy wrt u and w simultaneously\n");
    ReturnValueType returnValue =
        CGMinimization(function, tolerance, maxNumberIterations, debugLevel,
                       output_flag, loadNumber);
  ...
  case 1: {
    ...
    d_print("CG Minimization: Minimizing free energy wrt u and w in an "
            "alternate way\n");

    ReturnValueType returnValue = CGAlternateMinimization(
        function, tolerance, maxNumberIterations, debugLevel, output_flag,
        loadNumber, minMethodMaxIterations, minMethodTolerance);
  ...
  case 2: {
    ...
    if (returnValue == SUCCESS) {
      ...
      d_print("CG Minimization: Max iteration reached for frequency CG "
              "Minimization\n");
      d_print("CG Minimization: Exiting the code\n");
      exit(EXIT_FAILURE);
    }
  ...
  case 3: {
    ...
    if (returnValue == SUCCESS) {
      ...
      d_print("CG Minimization: Max iteration reached for position CG "
              "Minimization\n");
      d_print("CG Minimization: Exiting the code\n");
      exit(EXIT_FAILURE);
    }
...
CGNonLinearSolver::CGMinimization(SolverFunction &function, double tolerance,
                                  int maxNumberIterations, int debugLevel,
                                  int output_flag, int loadNumber) {
  //
  // method const data
  //
  const double toleranceSqr = tolerance * tolerance;
...
#endif // HAVE_MPI
  if (debugLevel >= 1)
    d_print("Iteration no. |  delta new  |  resdiual norm   |   residual norm "
            "avg \n");
...
//
    ...
    if (debugLevel >= 1)
      d_print(
          "CG Iteration : %d               %lf         %lf           %lf \n",
          d_iter, d_deltaNew, residualNorm, residualNorm / totalnumberUnknowns);
    ...
    //
    ReturnValueType lineSearchReturnValue = CGNonLinearSolver::lineSearch(
        function, d_lineSearchTolerance, d_lineSearchMaxIterations, debugLevel);
    ...
    d_beta = (lineSearchReturnValue == SUCCESS)
                 ? (d_deltaNew - d_deltaMid) / d_deltaOld
                 : 0.0;
...
//
NonLinearSolver::ReturnValueType CGNonLinearSolver::CGAlternateMinimization(
    SolverFunction &function, double tolerance, int maxNumberIterations,
    int debugLevel, int output_flag, int loadNumber, int minMethodMaxIterations,
    double minMethodTolerance) {
  //
  ...
  for (int d_minIter = 0; d_minIter < minMethodMaxIterations; d_minIter++) {
    ...
    returnValue = CGMinimization(function, tolerance, maxNumberIterations,
                                 debugLevel, output_flag, loadNumber);
    ...
    if (d_minIter > 0) {
      ...
      // check for convergence
      //
      d_print("Iteration = %d - Checking convergence for CG Alternate "
              "Minimization method\n",
              d_minIter);
      if (computeResidualL2Norm() < minMethodTolerance) {
  ...
  d_print(
      "CG Minimization: Max iteration reached for Alternate CG Minimization\n");
  d_print("CG Minimization: Exiting the code\n");
  exit(EXIT_FAILURE);
}
}

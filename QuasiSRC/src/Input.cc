...
namespace quasicontinuum {
  ...
  void Input::parameterInit()
  {
    ...
    //                               1 - equilibrated from crystal lattice configuration
          ...
          char     **av)
  {
    ...
    while( fgets(line, (int) (line_max-1L), init ) != NULL )
    {
      ...
      if(d_universalConst.first == 1)
      {
        ...
        {
          PairPotentials::getInstance()->setUniversalConstants(d_universalConst.second);
        }
      }
      ...
      if( strstr(line, keywords[OUTPUT_DIRECTORY]) != NULL )
      {
        ...
        if(outputFlag == 1)
        {
          ...
          Output::getInstance()->createOutputDirectory(outputFlag, outputDirectory);
        }
        else
        {
      ...
      {
        sscanf(line, "%*s %lf %lf %lf", 
          &d_indentInitialLocation[0],
          &d_indentInitialLocation[1],
      ...
      if( strstr(line, keywords[VOID_TYPE]) != NULL )
      {
        ...
        if(voidC->isVoidEnable() == 1)
        {
          ...
          if(d_voidType[0] == 'C')
          {
            ...
            voidC->setVoidParameters(center, type, 
              d_voidNumParams, d_voidParams);
          }
      ...
      {
        sscanf(line, "%*s %lf %lf %lf",  
          &d_deformationMatrix[0][0],
          &d_deformationMatrix[0][1],
          &d_deformationMatrix[0][2]);
      }

      if( strstr(line, keywords[DEF2]) != NULL )
      {
        sscanf(line, "%*s %lf %lf %lf",  
          &d_deformationMatrix[1][0],
          &d_deformationMatrix[1][1],
          &d_deformationMatrix[1][2]);
      }

      if( strstr(line, keywords[DEF3]) != NULL )
      {
        sscanf(line, "%*s %lf %lf %lf",  
          &d_deformationMatrix[2][0],
          &d_deformationMatrix[2][1],
          &d_deformationMatrix[2][2]);
      }
      ...
      if( strstr(line, keywords[CHARGE]) != NULL )
      {
        ...
        if(chargeNum==d_externalCharges.size())
          d_externalCharges.push_back(std::make_pair(charge_increment, 
            charge_location));
      }
      ...
      if( strstr(line, keywords[SHIFT]) != NULL )
      {
        ...
        {
          Quasicontinua::getInstance()->insertShift(shiftNum,
                  shifts,
                  shiftFlag);
                  numShifts++;
        }
      ...
      if( strstr(line, keywords[POTENTIAL]) != NULL )
      {
        ...
        {
          d_print("Error : Need to read electrostatics flag before calling LoadPotential()\n");
          exit(EXIT_FAILURE);
        }

        if(flags_for_LoadPotential[1] != 6)
        {
          d_print("Error : Need to read electro boundary conditions before calling LoadPotential()\n");
          exit(EXIT_FAILURE);
        }
    ...
    if(d_residualForceRemoveFlag.size() != 0)
      ForceEnergyCalculation::getInstance()->setRemoveResidualFlags(d_residualForceRemoveFlag);       
...
struct qc_options_t Input::quasiInput(const int iQuasi,
          ...
          char                  **av)
  {
    ...
    initial_freq = sqrt(atomicMass * d_universalConst.second[0] * d_temperature)/ d_tau;
    ...
    if (d_qcOption.restart == OFF)
    {
      ...
      if (quasicontinuum_id > 1)
      {
        ...
        //if( (str2 = malloc((strlen(data_file) - data_file_count)*sizeof(har))) == NULL ){
    ...
    else
    {
      ...
      if(quasicontinuum_id > 1)
      {
        ...
        if( (str2 = (char *) malloc((strlen(d_restartFile)-restart_file_count)*sizeof(char))) == NULL )
        {
          ERROR("malloc()");
          exit(EXIT_FAILURE);
        }
  ...
  //
  void
  Input::LoadPotential(const int potentialType)
  {
    //
    //  potential type 
    //  0 : LJ
    ...
    //        
    long double dielectricConstant = 4.0 * M_PI * PairPotentials::getInstance()->getElectricConstant();
    ...
    switch(potentialType)
    {
      ...
      case 0:
      {
        ...
        //
        pairPotentials->insertEAMPotential(interactionData,
                potentialType,
                parameterData,
                "Mishin02_NiNi.eam",
                nGridPoints,
        ...
        if(d_electrostaticFlag == 1)
        {
          ...
          electrostaticsC->insertInputData(charges,
                  d_electrostaticCutoffRadius,
                  elementSize,
                  electricConstant,
                  d_electrostaticIntegration,
      ...
      case 1:
      {
        ...
        if(d_latticeInfo.first == 0)
        {
          for(int i=0; i< d_latticeInfo.second.size(); i++)
          {
            if(d_latticeInfo.second[i].first%2 == 0)
            {
              ...
              d_latticeInfo.second[i].second.second = 1.0; // core-mass is irrelevant in present code
          }
        }
        ...
        for (int n = 0; n < 12; ++n)
        {
          ...
          // insert 1st potential information into singleton
          //
          pairPotentials->insertNonEAMPotential(interactionData,
               potentialType,
               parameterData,
               EAMFlag);
        }
      ...
      case 3:
      {
        ...
        if(d_latticeInfo.first == 0)
        {
          for(int i=0; i< d_latticeInfo.second.size(); i++)
          {
            if(d_latticeInfo.second[i].first == 0 || d_latticeInfo.second[i].first == 1)
...
//  ProcessOptions()
//
void Input::ProcessOptions(char         **init_file,
        char         **data_file,
        int           *number_threads,
...
//
int Input::CreateDefaultInitFilename(char        *init_file,
                size_t              n,
                char                *name,
                char const          *ext)
{
  ...
  if( strlen(local_base_name) + strlen(ext) >= n ) return(1);
...
void Input::ReadQuasiData(const int           quasicontinuum_id,
                    ...
                    char                        *input_filename)
  {
    ...
    /**
      ...
      */
    memset( (void *) &P_node_list->new_node_list, 0, 
      sizeof(struct node_list_t) );

    pthread_mutex_init( &P_node_list->node_list.lock, 
    ...
    for(i_node=0; i_node < P_node_list->node_list.number_nodes; i_node++)
    {
      ...
      latticeClass->getSiteInitialPosition(P_node->initial_position,
        P_node->l, P_lattice, quasicontinuum_id);
  ...
  void Input::systemInfoOutput()
  {
    ...
    for(int i=0; i < 3; i++)
    {
      for(int j=0; j < 3; j++)
      {
        d_configurationMinMax.first[i] += d_latticeMinMax.first[j]*d_latticeConstants[i][j];
        
        d_configurationMinMax.second[i] += d_latticeMinMax.second[j]*d_latticeConstants[i][j];
      }
    }
    ...
    if(d_electrostaticFlag == 1)
    {
      ...
      if(i1 == 1 && i2 == 1 && i3 == 1)
        fprintf(file, "charge neutral, free surface and specified charge density mixed bc ");
    ...
    if(d_minMethod == 0)
    {
      fprintf(file, " Minimization method              Minimization wrt u and w simultaneously\n");
    }
    else if(d_minMethod == 1)
    {
      fprintf(file, " Minimization method                 Minimization wrt u and w alternatively\n");      
      fprintf(file, " Minimization Method MaxIter                 %d\n", d_minMethodMaxIter);
      fprintf(file, " Minimization Tolerance                      %f\n", d_minMethodTol);      
    }
    else if(d_minMethod == 2)
    {
      fprintf(file, " Minimization method                 Minimization wrt w keeping u fixed at initial state\n");
    }
    else if(d_minMethod == 3)
    {
      fprintf(file, " Minimization method                 Minimization wrt u keeping w fixed at initial state\n");
    }
    else
    {
    ...
    fprintf(file, "   %lf     %lf     %lf\n", 
      ...
      d_latticeConstants[0][1], 
      d_latticeConstants[0][2]);
    fprintf(file, "   %lf     %lf     %lf\n", 
      d_latticeConstants[1][0], 
      d_latticeConstants[1][1], 
      d_latticeConstants[1][2]);
    fprintf(file, "   %lf     %lf     %lf\n", 
      d_latticeConstants[2][0], 
      d_latticeConstants[2][1], 
      d_latticeConstants[2][2]);

    fprintf(file, " Lattice Size \n");
    fprintf(file, "   l_min =  %d    %d    %d\n", 
      d_latticeMinMax.first[0],
      d_latticeMinMax.first[1],
      d_latticeMinMax.first[2]);    
    fprintf(file, "   l_max = %d    %d    %d\n", 
      d_latticeMinMax.second[0],
      d_latticeMinMax.second[1],
      d_latticeMinMax.second[2]);    

    fprintf(file, " Initial Configuration Size\n"); 
    fprintf(file, "   x_min = %f        %f        %f\n",
      d_configurationMinMax.first[0],
      d_configurationMinMax.first[1],
      d_configurationMinMax.first[2]);   
    fprintf(file, "   x_max = %f        %f        %f\n",
      d_configurationMinMax.second[0],
      d_configurationMinMax.second[1],
      d_configurationMinMax.second[2]);   

    fprintf(file, " Lattice Shift\n");
    fprintf(file, "     Lattice Number      shift_x       shift_y       shift_z       core       atomic mass\n");
    for(int i=0; i < d_shift.size(); i++)
      fprintf(file, "     %d                  %f        %f       %f        %d        %lf\n",
        i+1, 
        d_shift[i][0],
        d_shift[i][1],
        d_shift[i][2],
    ...
    if(d_initialConfigurationFlag == 0)
      fprintf(file, " Initial Configuration                 Crystal lattice configuration \n");
    else if(d_initialConfigurationFlag == 1)
      fprintf(file, " Initial Configuration                 Equilibrated from crystal lattice configuration \n");
    else
      fprintf(file, " Initial Configuration                 Specified by input file \n"); 
    ...
    else
    {
      ...
      if(d_externalCharges.size() != 0)
      {
        ...
        fprintf(file, "     Charge Number       increment         x           y            z\n");
        for(int i=0; i < d_externalCharges.size(); i++)
        fprintf(file, "     %d                    %f    %f   %f      %f\n",
            i+1,
            d_externalCharges[i].first,
            d_externalCharges[i].second[0],
            d_externalCharges[i].second[1],
    ...
    else  
    {
      ...
      for(int i=0; i<3; i++)
        fprintf(file, "   %lf     %lf     %lf\n",
          ...
          d_deformationMatrix[i][1],
          d_deformationMatrix[i][2]);
    }
    ...
    fprintf(file, " All     xBegin     xEnd     yBegin     yEnd     zBegin     zEnd\n");
  ...
  //  ReadRestartFile()
  //
  void Input::ReadRestartFile(int                    iQuasi,
      const char                              *filename,
      ...
      char *                                  materialName)
  {
    ...
    {    
       ...
       */
      if(xdr_int(&xdrs, &tet[0])==FALSE ||
         xdr_int(&xdrs, &tet[1])==FALSE ||
         xdr_int(&xdrs, &tet[2])==FALSE ||
         xdr_int(&xdrs, &tet[3])==FALSE)
        XDR_ERR("xdr_int()");
      ...
      P_element_list->elements[i_elem]->cir_radius = 
        miscC->findTetraCenter( P_node_list->nodes[tet[0]]->initial_position,
                     P_node_list->nodes[tet[1]]->initial_position,
                     P_node_list->nodes[tet[2]]->initial_position,
                     P_node_list->nodes[tet[3]]->initial_position,
                     center);
  ...
  //  ReadAtomicMass()
  //
  void Input::ReadAtomicMass(int             iQuasi,
      double                            &mass)  
  {

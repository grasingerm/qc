...
//
void ProcessClusterSiteLocal(
    pair_state_t iC_state, vec_int_t iC_Info, const int iQuasi,
    const int numQuasi,
    std::vector<std::vector<std::vector<neigh_site_data_t>>> *P_neighborList,
  ...
  for (jQuasi = 0; jQuasi < numQuasi; jQuasi++) {
    ...
    if (potentialNumber != -1) {
      // quasicontinuum interacts and thus, call PairPotentials to get cutoff
      // radius
    ...
    if (potentialNumber != -1) {
      ...
      for (jNSite = 0; jNSite < jNeighSitesAndStates.size(); jNSite++) {
        ...
        if (Index_jNSite == -1) {
          ...
          // site is found
          (*P_neighborList)[jQuasi][jBucket][Index_jNSite]
              .second[iQuasi]
              .push_back(iC_Info);
        } // site found
...
void *ProcessAllClusterOfNodeWorker(void *arg) {
  ...
  const int iNode = ((struct ProcessAllClusterOfNodeData_t *)arg)->iNode;
  ...
  get_share(my_id, number_threads, P_iCSites->number_sites, &number_thread,
            &cluster_site_start, &cluster_site_end);
  ...
  for (i_cluster = cluster_site_start; i_cluster <= cluster_site_end;
       i_cluster++) {
    ...
    latticeC->getSiteCurrentStateAndNodeInfo(iC_data, P_iQuasi_lattice, iQuasi);
    ...
    int i_bucket = GetBucketNumberLocal(iC_data.first.first);

    // check if cluster site is already in cluster data bucket
    pthread_mutex_lock(&(bucket_lock[i_bucket]));
    int index_iC = FindSiteInClusterBucketLocal(
        (*P_clusterData)[iQuasi][i_bucket], iC_data.first.first);
    pthread_mutex_unlock(&(bucket_lock[i_bucket]));

    // index_iC must be -1 because by design no node should have common cluster
    // site
    if (index_iC != -1) {
    ...
    if (pairC->d_EAMFlag == true && coreShellType == -1)
      ProcessClusterSiteLocal(iC_data.first, iC_Info, iQuasi, numQuasi,
                              P_neighborList, thread_flag);
...
void *ComputeNeighborFromCurrentClusterWorker(void *arg) {
  ...
  get_share(my_id, number_threads, numBuckets, &number_thread, &bucket_start,
            &bucket_end);
  ...
  for (iBucket = bucket_start; iBucket <= bucket_end; iBucket++) {
    for (iCluster = 0; iCluster < (*P_clusterData)[iQuasi][iBucket].size();
      ...
      for (int dof = 0; dof < 3; dof++)
        iC_site.push_back(
            (*P_clusterData)[iQuasi][iBucket][iCluster].first.first[dof]);

      // pthread_mutex_lock(&lock_pint);
      // std::cout<<"processing cluster site "<<iBucket<<"
      // "<<iCluster<<std::endl;
      // pthread_mutex_unlock(&lock_pint);

      latticeC->getSiteCurrentStateNew(iC_state, iC_site, iQuasi);
      ...
      if (pairC->d_EAMFlag == true && coreShellType == -1)
        ProcessClusterSiteLocal(
            (*P_clusterData)[iQuasi][iBucket][iCluster].first, iC_Info, iQuasi,
            numQuasi, P_neighborList, thread_flag);
...
//
//
void UpdateClusterLocationsLocal(
    const int iQuasi, vec_dbl_t iShift, const int bucket_start,
  ...
  for (iBucket = bucket_start; iBucket <= bucket_end; iBucket++) {
    for (iCluster = 0; iCluster < (*P_clusterBucket)[iBucket].size();
      ...
      for (int dof = 0; dof < 3; dof++)
        iC_site.push_back(
            (*P_clusterBucket)[iBucket][iCluster].first.first[dof]);
...
//
//
void UpdateNeighborLocationsLocal(
    const int iQuasi, vec_dbl_t iShift, const int bucket_start,
    const int bucket_end,
    std::vector<std::vector<neigh_site_data_t>> *P_neighborBucket) {
  int iBucket;
  ...
  for (iBucket = bucket_start; iBucket <= bucket_end; iBucket++) {
    for (iNeighbor = 0; iNeighbor < (*P_neighborBucket)[iBucket].size();
      ...
      for (int dof = 0; dof < 3; dof++)
        iN_site.push_back(
            (*P_neighborBucket)[iBucket][iNeighbor].first.first[dof]);

      latticeC->getSiteCurrentStateNew(iN_state, iN_site, iQuasi);
...
void *UpdateClusterLocationsWorker(void *arg) {
  ...
  UpdateClusterLocationsLocal(iQuasi, iShift, bucket_start, bucket_end,
                              P_clusterBucket);
...
void *UpdateNeighborLocationsWorker(void *arg) {
  ...
  // call function to process range of buckets assigned to current thread
  UpdateNeighborLocationsLocal(iQuasi, iShift, bucket_start, bucket_end,
                               P_neighborBucket);
...
void CrossNeighborList::computeCrossNeighborList(
  ...
  if (rebuild_cluster_flag == 1) {
    ...
    ComputeNeighborList(mt_version, rebuild_cluster_flag,
                        rebuild_neighbor_flag);
  ...
  if (RunData::getInstance()->d_timeOut == true) {
    for (iQuasi = 0; iQuasi < numQuasi; iQuasi++) {
      ...
      if (rebuild_cluster_flag == 1) {
        ...
        elapsedTime +=
            (buildTime_t[t_2].tv_usec - buildTime_t[t_1].tv_usec) / 1000000.0;
        RunData::getInstance()->addComputeClusterAndNeighborListTime(
            iQuasi, elapsedTime);
      } else {
        int t_1 = iQuasi * 2 + 0;
        int t_2 = iQuasi * 2 + 1;
        ...
        elapsedTime +=
            (neighborList_t[t_2].tv_usec - neighborList_t[t_1].tv_usec) /
            1000000.0;

        RunData::getInstance()->addBuildNeighborListTime(iQuasi, elapsedTime);
      }
  ...
  case SINGLE_THREADED: {
    ...
    for (iNode = 0; iNode < all_node_list.node_list.number_nodes; iNode++) {
      ...
      for (i_cluster = 0; i_cluster < iCSites.number_sites; i_cluster++) {
        ...
        // read data from Lattice class function
        latticeC->getSiteCurrentStateAndNodeInfo(iC_data, &iQuasi_lattice,
                                                 iQuasi);
        ...
        // check if cluster site is already in cluster data bucket
        int index_iC = FindSiteInClusterBucketLocal(
            d_clusterData[iQuasi][i_bucket], iC_data.first.first);

        // index_iC must be -1 because by design no node should have common
        // cluster site
        if (index_iC != -1) {
          d_print("Error in cluster data and node's cluster data\n");
    ...
    for (iNode = 0; iNode < all_node_list.node_list.number_nodes; iNode++) {
      ...
      if (iCSites.number_sites >= number_threads) {
        ...
        for (i_cluster = 0; i_cluster < iCSites.number_sites; i_cluster++) {
          ...
          //
          // now compute all neighbor sites of i_cluster
          //  Note : call this only if EAM is enabled and if quasi is of shell
          //  type
          //
  ...
  case SINGLE_THREADED: {
    ...
    for (iBucket = 0; iBucket < numBuckets; iBucket++) {
      for (iCluster = 0; iCluster < d_clusterData[iQuasi][iBucket].size();
        ...
        for (int dof = 0; dof < 3; dof++)
          iC_site.push_back(
              d_clusterData[iQuasi][iBucket][iCluster].first.first[dof]);
        ...
        if (pairC->d_EAMFlag == true && coreShellType == -1)
          ProcessClusterSiteLocal(
              d_clusterData[iQuasi][iBucket][iCluster].first, iC_Info, iQuasi,
              numQuasi, &d_neighborList, thread_flag);
      } // loop over cluster sites
    }   // loop over buckets
  }     // single threaded
...
void CrossNeighborList::UpdateLocation(enum mt_version_t mt_version) {
  ...
  case SINGLE_THREADED: {
    ...
    for (int iQuasi = 0; iQuasi < numQuasi; iQuasi++) {
      ...
      UpdateClusterLocationsLocal(iQuasi, iShift, bucket_start, bucket_end - 1,
      ...
      if (pairC->d_EAMFlag == true) {
        ...
        UpdateNeighborLocationsLocal(iQuasi, iShift, bucket_start,
                                     bucket_end - 1, &(d_neighborList[iQuasi]));
      }
      ...
      if (RunData::getInstance()->d_timeOut == true)
        RunData::getInstance()->addUpdateClusterAndNeighborListTime(
            iQuasi, elapsedTime);
    } // loop over iQuasi
  }   // single threaded
...
CrossNeighborList::getNodeInfoOfClusterSite(int iQuasi,
  ...
  if (iQuasi > d_clusterData.size() || iQuasi < 0) {
    d_print(
        "wrong iQuasi passed to CrossNeighborlist::getNodeInfoOfClusterSite\n");
  ...
  if (index == -1) {
    d_print("site (%d, %d, %d) of quasi %d not found in cluster data\n",
            clusterSite[0], clusterSite[1], clusterSite[2], iQuasi);

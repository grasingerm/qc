...
extern int __pm_get_page_info(void *base_addr,
			      size_t length,
...
static int 
compare_devs(const void *dev1,
	     const void *dev2)
{

  if (*((dev_t *)dev1) == *((dev_t *)dev2)) return (0);
...
/* ARGSUSED4 */
void
get_area_placement(caddr_t vaddr,
		   size_t  n_elems,
		   size_t  elem_size,
		   dev_t  *dev_list,
		   int     dev_list_len,
		   int    *n_devs)
{
  ...
  if (n_pages <= PGINFO_MAX_NUM_PAGES){
    ...
    if ((n_pages_read=__pm_get_page_info(vaddr, n_elems*elem_size, pginfo_buf,
					 n_pages)) < 0){
    ...
    if ((n_pages_read=__pm_get_page_info(addr_start, vaddr+n_elems*elem_size-
					 addr_start, pginfo_buf, 
					 n_pages)) < 0){
      ERROR("__pm_get_page_info()");
      exit(EXIT_FAILURE);
    }
...
void
create_locality_domain(int number_threads)
{
  ...
  if (mldset_place(mldset, TOPOLOGY_FREE, NULL, 0, RQMODE_MANDATORY)
      < 0){
    ERROR("mldset_place()");
    exit(EXIT_FAILURE);
  }
...
void
change_memory_placement(caddr_t vaddr,
			size_t  size,
			int     number_threads)
{
...
bind_to_spu(pthread_t tid)
{
  ...
  if (pthread_processor_bind_np(PTHREAD_BIND_FORCED_NP, &answer,
				current_spu, tid)) {

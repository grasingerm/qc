...
namespace quasicontinuum {
  ...
  //
  void
  Quasicontinua::insert(int    num_quasi,
			int    ac,
			char * av[])
  {
    ...
    for (int i = 0; i < num_quasi; i++){
      ...
      //
      const Quasicontinuum quasicontinuum_instance(i,
                ac,
						    av);
  ...
  const Quasicontinuum & 
  Quasicontinua::get(id_type id) const
  {
    ...
    //
    assert(id >= 0 && id < d_quasicontinua.size());
  ...
  const Quasicontinuum & 
  Quasicontinua::getQuasi(id_type id) const  
  {
    //
    // check validity of id
    //
  ...
  Quasicontinuum & 
  Quasicontinua::getQuasi(id_type id)
  {
    //
    // check validity of id
    //
  ...
  Quasicontinua::size_type 
  Quasicontinua::size() const
  {
  ...
  Quasicontinua::id_type 
  Quasicontinua::getCurrentId() const
  {
  ...
  //
  void 
  Quasicontinua::insertShift(int                 quasicontinuumId,
         ...
         int                 coreShell)
  {
    ...
    // pair to hold data
    //
    std::pair<int,std::vector<double> > data(quasicontinuumId,
               shiftVector);
  ...
  //
  std::vector<double> 
  Quasicontinua::getRelativeShift(unsigned int iQuasi,
             unsigned int jQuasi)
  {
    ...
    //
    assert(iQuasi < d_shifts.size() && iQuasi >= 0);
    assert(jQuasi < d_shifts.size() && jQuasi >= 0);
    ...
    for (unsigned int iShift = 0; iShift < d_shifts.size(); ++iShift){
      ...
      //
      // check if iQuasi matches
      //
      if (int(iQuasi) == d_shifts[iShift].first){
  ...
  shiftOne = d_shifts[iShift].second;
      ...
      //
      // check if jQuasi matches
      //
      if (int(jQuasi) == d_shifts[iShift].first){
  ...
  //
  std::vector<double> 
  Quasicontinua::getShift(unsigned int QuasicontinuumID)
  {

    //
    // check QuasicontinuumID against size of d_shifts
    //
    assert(QuasicontinuumID < d_shifts.size() && QuasicontinuumID >= 0);
  ...
  //
  int
  Quasicontinua::getCoreShell(unsigned int QuasicontinuumID)
  {
  ...
  //  getTemperature()
  //
  double Quasicontinua::getTemperature()
  {
  ...
  std::vector<double> Quasicontinua::getSigmaVector(void)
  {
    ...
    for(int iQuasi = 0; iQuasi < d_quasicontinua.size(); iQuasi++)
    {

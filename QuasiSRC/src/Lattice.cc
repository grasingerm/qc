...
namespace quasicontinuum {
  ...
  //  findClosestSite()
  //
  void Lattice::findClosestSite(int                l[3],
                  double                    X[3],
                  ...
                  const int               iQuasi)
  {
    if(Void::getInstance()->isVoidEnable() != 0)
    {
      ...
      if(Void::getInstance()->findSiteInVoidCache(site, iQuasi) == RETURN_SUCCESS)
  ...
  //  isLatticeSite()
  //
  int Lattice::isLatticeSite(const int               l[3],
               const struct lattice_t *P_lattice,
               const int               iQuasi)  
  {
  ...
  {
    if(memcmp(l1, l2, sizeof(int[3])) == 0) return(RETURN_SUCCESS);
  ...
  //
  void Lattice::getSiteInitialPosition(double                  r[3],
         const int               l[3],
         ...
         const int               iQuasi)
  {
    ...
    else
    {
      ...
      // get element
      P_element = Element::getInstance()->locateSiteElement(&(shape[0]), 
                    l, P_lattice, iQuasi);
      ...
      // compute position
      r[0] = 
        P_node_0->initial_position[0]*shape[0]+P_node_1->initial_position[0]*shape[1]+
        P_node_2->initial_position[0]*shape[2]+P_node_3->initial_position[0]*shape[3];

      r[1] =
        P_node_0->initial_position[1]*shape[0]+P_node_1->initial_position[1]*shape[1]+
        P_node_2->initial_position[1]*shape[2]+P_node_3->initial_position[1]*shape[3];

      r[2] =
        P_node_0->initial_position[2]*shape[0]+P_node_1->initial_position[2]*shape[1]+
        P_node_2->initial_position[2]*shape[2]+P_node_3->initial_position[2]*shape[3];
    ...
    //
    void Lattice::getSiteInitialFrequency(double                  &f,
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)
    {
      ...
      f = 
        P_node_0->initial_frequency*shape[0]+P_node_1->initial_frequency*shape[1]+
        P_node_2->initial_frequency*shape[2]+P_node_3->initial_frequency*shape[3];         
    ...
    //
    void Lattice::getSiteInitialTemperature(double                  &T,
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)
    {
    ...
    //  getSiteInitialState()
    //
    void Lattice::getSiteInitialState(double                  S[4],
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)
    {
    ...
    //
    void Lattice::getSiteInitialStateInterpolate(double                  S[5],
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)
    {
    ...
    //
    void Lattice::getSiteCurrentPosition(double                  r[3],
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)
    {
    ...
    //
    void Lattice::getSiteCurrentFrequency(double                  &f,
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)    
    {
    ...
    //
    void Lattice::getSiteCurrentTemperature(double                  &T,
           const int               l[3],
           struct lattice_t *P_lattice, 
           const int               iQuasi)    
    {
    ...
    //  getSiteCurrentPosition()
    //
    void Lattice::getSiteCurrentState(double                  s[4],
           const int               l[3],
           struct lattice_t *P_lattice,
           const int               iQuasi)
    {
    ...
    //
    void Lattice::getSiteCurrentStateAndNodeInfo(std::pair< std::pair< std::vector<int>, std::vector<double> >, std::vector<int> > &iC_data, 
        struct lattice_t   *P_lattice,
        const int               iQuasi)    
    {
      // clear the data
  ...
  //  getShellRadius()
  //
  double Lattice::getShellRadius(struct lattice_t *P_lattice,
                const int               l[3],
                const int               iQuasi) 
  {
    double X[3];
  ...
  //
  int Lattice::GenerateNewShell(int               shell_number,
     struct lattice_t  lattice)
    {
      ...
      case FCC:
        ...
        if(shell_number <= d_fcc_shells.size()) return(RETURN_OK);
      ...
      // check if shell_number is already in d_bcc_shells
        if(shell_number <= d_bcc_shells.size()) return(RETURN_OK);
  ...
  //
  int Lattice::FillShell(struct shell_t*      data_shell,
      int                            shell_number,
      struct lattice_t               lattice)
  {
    switch(lattice.type)
          ...
          for (l1=0; l1 <= l_max; l1++)
            for (l2=0; l2 <= l_max; l2++)
              for (l3=0; l3 <= l_max; l3++)
                if ((l1*l1+l2*l2+l3*l3 == d_fcc_p) && ((l1+l2+l3)%2 == 0))
                {
                  // add (l1,l2,l3)
  ...
  //  AddNewSite()
  //
  int Lattice::AddNewSite(struct shell_t*      data_shell,
      int                             l1,
      int                             l2,
      int                             l3)
  {
    ...
    else
    {
      ...
      data_shell->site = (int (*)[3]) realloc(data_shell->site,
        (data_shell->number_sites+1)*sizeof(int[3]));
    }

    if(data_shell->site == NULL) return(RETURN_ERR);
    ...
    //   std::cout<<"++++++"<<std::endl;
    //   std::cout<<l1<<" : "<<data_shell->site[data_shell->number_sites - 1][0]<<std::endl;
    //   std::cout<<l2<<" : "<<data_shell->site[data_shell->number_sites -1][1]<<std::endl;
    //   std::cout<<l3<<" : "<<data_shell->site[data_shell->number_sites -1][2]<<std::endl;
    // }      
  ...
  //
  void Lattice::GetLatticeCoordinates(double                  r[3],
          double                  X[3],
          const struct lattice_t *P_lattice)
  {
    MiscFunctions * miscF = MiscFunctions::getInstance();
    double det = miscF->computeDeterminant(P_lattice->a1, P_lattice->a2,
              P_lattice->a3);

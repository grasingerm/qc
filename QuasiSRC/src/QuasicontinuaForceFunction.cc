...
//
int QuasicontinuaForceFunction::getNumberUnknowns() const {
  //
  //
  //
...
//
double QuasicontinuaForceFunction::value() const {
  //
...
//
void QuasicontinuaForceFunction::gradient(
    std::vector<double> &gradient, const int rebuild_neighbor_flag) const {
  // Quasicontinua
  Quasicontinua *quasicontinua = Quasicontinua::getInstance();
...
//
void QuasicontinuaForceFunction::precondition(
    std::vector<double> &s, const std::vector<double> &gradient) const {
  //
  ...
  case quasicontinuum::SINGLE_THREADED: {
    ...
    for (int iQuasi = 0; iQuasi < numQuasi; ++iQuasi) {
      ...
      if (iQuasi == 0)
        ...
        startPrecondition += d_numQuasicontinuumUnknowns[iQuasi - 1];
      ...
      for (unsigned int i = 0; i < iPrecondition.size(); ++i) {
        ...
        s[startPrecondition + i] =
            -gradient[startPrecondition + i] / iPrecondition[i];
      }
    }
  }
...
// Update solution.
//
void QuasicontinuaForceFunction::update(const std::vector<double> &solution) {
  //
  // switch between versions
  //
  switch (d_version) {
  ...
  case quasicontinuum::SINGLE_THREADED: {
    ...
    // store solution
    //
    d_solution.insert(d_solution.end(), solution.begin(), solution.end());
    ...
    for (int iQuasi = 0; iQuasi < numQuasi; ++iQuasi) {
      ...
      for (int iCount = 0; iCount < d_numQuasicontinuumUnknowns[iQuasi];
           ++iCount)
...
//
void QuasicontinuaForceFunction::solution(std::vector<double> &solution) const {
  ...
  //
  solution.insert(solution.end(), d_solution.begin(), d_solution.end());
...
//
std::vector<int> QuasicontinuaForceFunction::getUnknownCountFlag() const {
  //
  //
  //
  return std::vector<int>(d_solution.size(),
                          1); // FIXME: not implemented for MPI
}

//
// get solution size
//
int QuasicontinuaForceFunction::getSolutionSize() const {
  //
  //
  //
  return d_solution.size();
...
//
void QuasicontinuaForceFunction::GetQuasicontinuaSolution() const {
  //
  // clear solution
  //
  d_solution.clear();
  ...
  for (unsigned int iQuasi = 0; iQuasi < numQuasi; ++iQuasi) {
    ...
    //
    d_solution.insert(d_solution.end(), iSolution.begin(), iSolution.end());
...
//	setPositionFixity()
//
void QuasicontinuaForceFunction::setPositionFixity(
    const int fixity_flag) const {
  //
  // get quasicontinua
  //
  Quasicontinua *quasicontinua = Quasicontinua::getInstance();
...
//	setFrequencyFixity()
//
void QuasicontinuaForceFunction::setFrequencyFixity(
    const int fixity_flag) const {
  //
  // get quasicontinua
  //

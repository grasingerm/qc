...
namespace quasicontinuum {
  ...
  void
  PairPotentials::destroyInstance()
  {
  ...
  //
  void
  PairPotentials::insertNonEAMPotential(std::pair<int,int>  quasicontinuumPair,
			 int                 potentialType,
  ...
  void
  PairPotentials::insertEAMPotential(std::pair<int,int>  quasicontinuumPair,
				  int                 potentialType,
				  std::vector<double> potentialParameters,
				  ...
				  const bool          EAMFlag)
  {
    ...
    for(int iPoint = 0; iPoint < NGridPoints; ++iPoint)
      fscanf(fp, "%le %le %le %le %le",
	     &(d_RhoX[potNumber-1][iPoint]),
	     &(d_RhoY[potNumber-1][iPoint]),
	     &(d_RhoB[potNumber-1][iPoint]),
	     &(d_RhoC[potNumber-1][iPoint]),
	     &(d_RhoD[potNumber-1][iPoint]));

    for(int iPoint = 0; iPoint < NGridPoints; ++iPoint)
      fscanf(fp, "%le %le %le %le %le",
	     &(d_EmbedX[potNumber-1][iPoint]),
	     &(d_EmbedY[potNumber-1][iPoint]),
	     &(d_EmbedB[potNumber-1][iPoint]),
	     &(d_EmbedC[potNumber-1][iPoint]),
	     &(d_EmbedD[potNumber-1][iPoint]));

    for(int iPoint = 0; iPoint < NGridPoints; ++iPoint)
      fscanf(fp, "%le %le %le %le %le",
	     &(d_PairX[potNumber-1][iPoint]),
	     &(d_PairY[potNumber-1][iPoint]),
	     &(d_PairB[potNumber-1][iPoint]),
  ...
  PairPotentials::getPairForceAndEnergy(int    potentialNumber,
							double separation)
  {
  ...
  //
  std::pair<double,double>
  PairPotentials::getDensityAndDerivative(int    potentialNumber,
						      double r)
  {
    ...
    switch(d_PotentialType[potentialNumber])
    {
      ...
      case 2:
      {
        ...
        if(r <= rCut)
        {
          ...
          SPLINE_F77(&nGrid,
            &r,
            &(d_RhoX[potentialNumber][0]),
            &(d_RhoY[potentialNumber][0]),
            &(d_RhoB[potentialNumber][0]),
            &(d_RhoC[potentialNumber][0]),
            &(d_RhoD[potentialNumber][0]),
            &density.first,
            &density.second);
        }
								...
								double density)
  {
    ...
    switch(d_PotentialType[potentialNumber])
    {
      ...
      case 2:
      {
        ...
        SPLINE_F77(&nGrid,
          &density,
          &(d_EmbedX[potentialNumber][0]),
          &(d_EmbedY[potentialNumber][0]),
          &(d_EmbedB[potentialNumber][0]),
          &(d_EmbedC[potentialNumber][0]),
          &(d_EmbedD[potentialNumber][0]),
          &embeddingFunction.first,
          &embeddingFunction.second);
  ...
  // LJ calculations
  //
  std::pair<double,double> PairPotentials::LJ(int    potentialNumber,
					      double r)
  {
						   ...
						   double r)
  {
    ...
    if(r <= rCut){
      ...
      SPLINE_F77(&nGrid,
		 &r,
		 &(d_PairX[potentialNumber][0]),
		 &(d_PairY[potentialNumber][0]),
		 &(d_PairB[potentialNumber][0]),
		 &(d_PairC[potentialNumber][0]),
		 &(d_PairD[potentialNumber][0]),
		 &forceEnergy.second,
		 &forceEnergy.first);
						  ...
						  double r)
  {
    ...
    if( r > d_PotentialParameters[potentialNumber][0]){
      ...
      // calculate force and energy
      //
      forceEnergy.first = 4.0 * epsilon * (-12.0 * sigma12 / (r12 * r) + 6.0 * sigma6 / (r6 * r));
						      ...
						      double r)
  {
    ...
    if( r > cutoffRadius){
      ...
      // calculate energy
      //
      forceEnergy.second = A * std::exp(-r / rho) - C / r6 - vC - vF * (r - cutoffRadius);
  ...
  //  getBoltzmanConstant()
  //
  double PairPotentials::getBoltzmanConstant(void)
  {
    return c_universalConst[1];
  }

  //
  //  getElectricConstant()
  //
  double PairPotentials::getElectricConstant()
  {
    return c_universalConst[4];
  }
  ...
  //
  void
  PairPotentials::getHarmonicApproxData(double       &phi,
            double                          &dphi,
            double                          &d_dphi,
            int                             potentialNumber,
            double                          separation)
  {
    ...
    case 6:
      ...
      // return force and energy values
      //
      return Anharmonic_Harmonic(phi,
        dphi,
        d_dphi,
        potentialNumber,
        separation);
  ...
  // LJ calculations
  //
  void PairPotentials::LJ_Harmonic(double       &phi,
            double                          &dphi,
            double                          &d_dphi,
            int                             potentialNumber,
            double                          r)
  {
    ...
    else
    {
      ...
      d_dphi = eps_4*12.0*(13.0*sigma_12/(r12*r*r) - 7.0*sigma_6/(r6*r*r));
    }
  ...
  // LJNiMn calculations
  //
  void PairPotentials::LJNiMn_Harmonic(double       &phi,
            double                          &dphi,
            double                          &d_dphi,
            int                             potentialNumber,
            double                          r)
  {
    ...
    else
    {
      ...
      dphi = 4.0*epsilon*(-12.0*sigma12/(r12*r) + 6.0*sigma6/(r6*r));
      d_dphi = 4.0 * epsilon * (12.0*13.0*sigma12/(r12*r*r) - 6.0*7.0*sigma6/(r6*r*r));
    }
  ...
  // Buckingham calculations
  //
  void PairPotentials::Buckingham_Harmonic(double       &phi,
            double                          &dphi,
            double                          &d_dphi,
            int                             potentialNumber,
            double                          r)
  {
    ...
    else
    {
      ...
      d_dphi = (A*std::exp(-r/rho))/(rho*rho) - 6.0*7.0*C/(r6*r*r);
    }
  ...
  // Harmonic calculations
  //
  void PairPotentials::Harmonic_Harmonic(double       &phi,
            double                          &dphi,
            double                          &d_dphi,
            int                             potentialNumber,
            double                          r)
  ...
  // Anharmonic calculations
  //
  void PairPotentials::Anharmonic_Harmonic(double       &phi,
            double                          &dphi,
            double                          &d_dphi,
            int                             potentialNumber,
            double                          r)

...
// addElementLocal()
//
void addElementLocal(struct element_t *P_element, struct node_t *P_node) {
  ...
  if (P_node->element_list.alloc_space == 0) {
    if ((P_node->element_list.elements = (struct element_t **)malloc(
             ELEMENT_ALLOC_CHUNK * P_elem_len)) == NULL) {
      PERROR_MALLOC();
      ERROR("xmalloc()");
      exit(EXIT_FAILURE);
    }
    ...
    // realloc the space
    P_node->element_list.elements = (struct element_t **)realloc(
...
// addNodeLocal()
//
void addNodeLocal(struct node_t *P_node, struct node_t *P_node_add) {
  ...
  if (P_node->node_list.alloc_space == 0) {
    if ((P_node->node_list.nodes =
             (struct node_t **)malloc(NODE_ALLOC_CHUNK * P_node_len)) == NULL) {
      PERROR_MALLOC();
      ERROR("malloc()");
      exit(EXIT_FAILURE);
    }
    ...
    P_node->node_list.nodes = (struct node_t **)realloc(
        P_node->node_list.nodes, P_node->node_list.alloc_space * P_node_len);
...
void *binElementsWorker(void *arg) {
  ...
  binElementsProcessingLocal(P_element_list, P_node_list, number_node_start,
                             number_node_end);
...
void Element::makeActiveElement(struct element_list_t *P_element_list,
  ...
  if (P_element_list->alloc_space < i_elem + 1) {
    ...
    if (P_element_list->alloc_space == 0)
      ...
      P_element_list->elements = (struct element_t **)realloc(
          P_element_list->elements,
...
#if defined(__QC_SGI)
    P_element_list->elements[i_elem] = (struct element_t *)memalign(
        page_size, elem_bucket * sizeof(struct element_t));
...
void Element::makeInactiveElement(struct element_list_t *P_element_list,
  ...
  if (P_element_list->alloc_inactive_space < i_elem + 1) {
    if (P_element_list->alloc_inactive_space == 0)
      ...
      P_element_list->inactive_elements = (struct element_t **)realloc(
          P_element_list->inactive_elements,
...
//
void Element::initializeSiteElementMapData(
    const int iQuasi, struct element_list_t *P_element_list) {
...
//  locateSiteElement()
//
struct element_t *Element::locateSiteElement(double *shape, const int l[3],
...
//
void Element::computeElementNumberSites(struct element_list_t *P_element_list,
  ...
  case FCC:
    atomic_volume =
        2.0 * P_lattice->a1[0] * P_lattice->a1[0] * P_lattice->a1[0];
    break;
  ...
  for (i_elem = 0; i_elem < P_element_list->number_elements; i_elem++) {
    ...
    P_element->number_sites = fabs(MiscFunctions::getInstance()->tetraVolume(
...
//
void Element::createElementsFromMeshData(struct element_list_t *P_element_list,
                                         struct node_list_t *P_node_list,
                                         struct lattice_t *P_lattice,
  ...
  for (int i_elem = 0; i_elem < P_mesh_data->number_tets; i_elem++) {
    ...
    P_element_list->elements[i_elem]->cir_radius =
        MiscFunctions::getInstance()->findTetraCenter(
            P_node_list->nodes[node_0]->initial_position,
            P_node_list->nodes[node_1]->initial_position,
            P_node_list->nodes[node_2]->initial_position,
            P_node_list->nodes[node_3]->initial_position, center);
...
void Element::computeDeformGrad(struct element_t *P_element) {
  ...
  shapeC->computeShapeFunctionGradient(
      P_node1->initial_position, P_node2->initial_position,
      P_node3->initial_position, P_node4->initial_position, grad_1);

  shapeC->computeShapeFunctionGradient(
      P_node2->initial_position, P_node1->initial_position,
      P_node3->initial_position, P_node4->initial_position, grad_2);

  shapeC->computeShapeFunctionGradient(
      P_node3->initial_position, P_node2->initial_position,
      P_node1->initial_position, P_node4->initial_position, grad_3);

  shapeC->computeShapeFunctionGradient(
      P_node4->initial_position, P_node2->initial_position,
      P_node3->initial_position, P_node1->initial_position, grad_4);
...
//
double Element::computeElementStrain(struct element_t *P_element, int i_load) {
  double(*F)[3];
  ...
  p = (P_element->strain[0] + P_element->strain[1] + P_element->strain[2]) / 3;
  ...
  return (P_element->strain[3] * P_element->strain[3] +
          P_element->strain[4] * P_element->strain[4] +
          P_element->strain[5] * P_element->strain[5] -
          deviator[0] * deviator[1] - deviator[0] * deviator[2] -

...
void Void::createVoid(struct node_list_t *P_node_list,
  ...
  case VPIT: {
    ...
    const double vertex1[3] = {d_voidCenter[0] - sideLength, d_voidCenter[1],
                               d_voidCenter[2]};

    const double vertex2[3] = {d_voidCenter[0] - 0.500 * sideLength,
                               d_voidCenter[1] + 0.866 * sideLength,
                               d_voidCenter[2]};
    ...
    const double vertex4[3] = {d_voidCenter[0] + sideLength, d_voidCenter[1],
                               d_voidCenter[2]};
    ...
    const double vertexBottom[3] = {d_voidCenter[0], d_voidCenter[1],
                                    d_voidCenter[2] - pitDepth};
    ...
    for (i_node = 0; i_node < P_node_list->number_nodes; i_node++) {
      ...
      if ((miscC->checkPointInTetra(vertex1, vertex2, voidCenter, vertexBottom,
                                    currentPosition) == INSIDE) ||
          (miscC->checkPointInTetra(vertex2, vertex3, voidCenter, vertexBottom,
                                    currentPosition) == INSIDE) ||
          (miscC->checkPointInTetra(vertex3, vertex4, voidCenter, vertexBottom,
                                    currentPosition) == INSIDE) ||
          (miscC->checkPointInTetra(vertex4, vertex5, voidCenter, vertexBottom,
                                    currentPosition) == INSIDE) ||
          (miscC->checkPointInTetra(vertex5, vertex6, voidCenter, vertexBottom,
                                    currentPosition) == INSIDE) ||
          (miscC->checkPointInTetra(vertex6, vertex1, voidCenter, vertexBottom,
                                    currentPosition) == INSIDE)) {
        /**
          * set void mask
          */
...
//  isVoidEnable()
//
int Void::isVoidEnable() { return d_enableVoid; }

//
//  setVoidParameters()
//
void Void::setVoidParameters(const double center[3], const enum void_t voidType,
...
//  insertVoidSiteCache()
//
void Void::insertVoidSiteCache(const std::vector<int> site, const int iQuasi) {
...
//  1 : site is void
//
int Void::findSiteInVoidCache(const std::vector<int> site, const int iQuasi) {
  //
  // loop over all sites in iQuasi
  //
  if (iQuasi < d_voidSiteCache.size()) {
    for (unsigned int iSite = 0; iSite < d_voidSiteCache[iQuasi].size();
         ++iSite) {
      //
      // see if site matches
      //
...
//  voidAtomsMissed()
//
std::vector<std::pair<int, std::vector<int>>> Void::voidAtomsMissed() {
  //
  // atoms to return
  //

...
namespace quasicontinuum {
  ...
  namespace
  {
    ...
    struct computePolarizationLocations_argument_t {
      ...
      std::vector<std::vector<std::pair<std::vector<double>,double> > > * elementPolarizationLocations;
      int                                                                 quasiId;
      double                                                              fixedCharge;
      struct lattice_t                                                  * lattice;
    };  
    ...
    struct computeChargeDensities_argument_t {
      ...
      std::vector<std::vector<std::pair<int,int> > >  * faceElementList; 
      struct node_list_t                              * nodeList;
      std::vector<std::vector<std::pair<double,std::vector<std::vector<double> > > > > * chargeDensity;
      std::vector<std::vector<double> >               * elementPolarization;
      struct lattice_t                                * lattice;
    ...
    struct computeChargedFace_argument_t {
      ...
      std::vector<std::vector<std::vector<int> > > *faces;
      std::vector<std::vector<std::pair<double,std::vector<std::vector<double> > > > > * charge_density;
    ...
    struct computeClusterChargeSurface_argument_t {
      ...
      double                                       *integration_error;
      struct node_list_t                           *zero_node_list;
      std::vector<std::vector<std::pair<std::vector<int>,int > > > *base_node_hash_list;
      int                                          *zero_quasi;
    };    
  ...
  namespace
  {
    ...
    //
    unsigned int getTripletIntegersKey(int a,
        int b,
        int c)
    {
    ...
    CalculateNormal(std::vector<std::vector<double> > nodeLocations)
    {
      ...
      // get cross product
      //
      std::vector<double> normal = MiscFunctions::getInstance()->crossProduct3x3(vec1,
                  vec2);
      //
    ...
    const int                       & quasi)
    {
      ...
      // get nodes in arrays
      //
      const double node1[3] = {nodeLocations[0][0],nodeLocations[0][1],nodeLocations[0][2]};
      const double node2[3] = {nodeLocations[1][0],nodeLocations[1][1],nodeLocations[1][2]};
      const double node3[3] = {nodeLocations[2][0],nodeLocations[2][1],nodeLocations[2][2]};
      const double node4[3] = {nodeLocations[3][0],nodeLocations[3][1],nodeLocations[3][2]};
      ...
      // get site location
      //
      Lattice::getInstance()->getSiteInitialPosition(location,
          site,
          &lattice,
          quasi);
      ...
      // check tetrahedron
      //
      if (MiscFunctions::getInstance()->checkPointInTetra(node1,
              node2,
              node3,
              node4,
              location) == INSIDE) 
    ...
    sitesInElement(const std::vector<int>                                  elementSite,
       const int                                               iElem,
       const std::vector<std::vector<std::vector<double> > > & atomisticElementLocations,
       std::vector<int>                                      & atomisticElements,
       struct lattice_t                                      & lattice,
       int                                             & quasi)
    {
      ...
      while(sitesAdded != 0)
      {
        ...
        //
        for (int siteCounter = 0; siteCounter < P_shell->number_sites; ++siteCounter)
        {
          //
          ...
          //
          if (Lattice::getInstance()->isSiteInsideLattice(&lattice, siteLattice, quasi) 
              == RETURN_SUCCESS)
          { 
            //
            // see if site is in element
            //
            if (isInElement(siteLattice,atomisticElementLocations[iElem],lattice, quasi) 
                == true)
            {
              //
    ...
    //
    void InsertNodeIntoHashList(const std::vector<int>  &site,
      ...
      switch(flag)
      {
        case 0:
        {
          ...
          for(quad = 0; quad < quadVectors.size(); quad++)
          {
            ...
            double r_hat_mag = sqrt(r_hat_ij[0] * r_hat_ij[0] + 
                  r_hat_ij[1] * r_hat_ij[1] + 
            ...
            if(r_hat_mag > 0.001)
            {
              for(int dof =0; dof < 3; dof++)
                r_hat_ij[dof] += factor_iSite * quadV_1[dof] - 
                                factor_jSite * quadV_2[dof];

              r_hat_mag = sqrt(r_hat_ij[0] * r_hat_ij[0] + 
                  r_hat_ij[1] * r_hat_ij[1] + 
                  r_hat_ij[2] * r_hat_ij[2] );
        ...
        case 3:
        {
          ...
          {
            for(int dof =0; dof < 3; dof++)
              field[dof] += electric_constant * source_charge 
                * r_hat_ij[dof] / r_hat_3;
    ...
    // add contribution from atom to electric field
    //
    void AddAtomicToFieldAndPotentialExternal(const std::vector<double> & iFieldState,
              const int                 & iQuasi,
              const std::vector<double> & jExternalCharge,
              const double & electric_constant,
              const double & source_charge,
              std::vector<double> & field,
              double & potential,
              int      iCoreFlag)    
    {
      ...
      if(iCoreFlag == -1)
      {
        ...
        for(quad = 0; quad < quadVectors.size(); quad++)
        {
          ...
          for(int dof =0; dof < 3; dof++)
            r_hat_ij.push_back(iFieldState[dof] + factor_iSite * quadVector[dof] - jExternalCharge[dof]);
          ...
          if(r_hat_mag > 0.1)
          {
            ...
            for(int dof =0; dof < 3; dof++)
              field[dof] += electric_constant * factor_quad * 
                source_charge * r_hat_ij[dof] * quadWeight / r_hat_3;
            ...
            potential += electric_constant * factor_quad * source_charge * quadWeight / r_hat_mag;
          }
        }
      }
      else
    ...
    //
    bool
    isSiteAtomistic(const std::vector<int>                               site,
        const std::vector<std::vector<std::vector<int> > > & atomisticCharges)
    {
      //
      // get bucket
      //
      unsigned int bucket = getTripletIntegersKey(site[0],
              site[1],
              site[2]);
    ...
    //
    const bool IsSiteANode(const std::vector<int> &site,
    ...
    //
    std::vector<double>
    midPointLine(const std::vector<double> &location1,
      ...
      const std::vector<double> location3)
    {
      ...
      // m12
      //
      std::vector<double> midLocation12 =
        midPointLine(location1,
         location2);

      //
      // m23
      //
      std::vector<double> midLocation23 =
        midPointLine(location2,
         location3);

      //
      // m13
      //
      std::vector<double> midLocation13 =
        midPointLine(location1,
         location3);
    ...
    fieldFromTriangularFace(double                            chargeDensity,
          std::vector<std::vector<double> > planeLocations,
          std::vector<double>               fieldLocation,
          double                            electricConstant,
          int                               method)
    {
      #if 0
      ...
      switch(method)
      {
        ...
        case 0:
        {
          ...
          //
          double area = miscC->TriangleArea(planeLocations[0],
                      planeLocations[1],
                      planeLocations[2]);
          ...
          //
          std::vector<double> center = miscC->TriangleCenter(planeLocations[0],
                           planeLocations[1],
                           planeLocations[2]);
      ...
      // get area of triangle
      //
      double J = miscC->TriangleArea(planeLocations[0],
               planeLocations[1],
               planeLocations[2]);
      ...
      for(unsigned int iQuad = 0; iQuad < localQuadPoints.size(); ++iQuad)
      {
        ...
        for(int iDof = 0; iDof < 3; ++iDof)
          for(int iShape = 0; iShape < 3; ++iShape)
              globalLocation[iDof] += planeLocations[iShape][iDof] * localQuadPoints[iQuad][iShape];
        ...
        //
        for(int iDof = 0; iDof < 3; ++iDof)
          field[iDof] += electricConstant * chargeDensity * J * localWeights[iQuad] * r[iDof] / r3;

        //
        // add to potential
        //
        potential += electricConstant * chargeDensity * J * localWeights[iQuad] / r1;
      }
    ...
    //
    std::pair<std::vector<double>, double>
    fieldFromDensityFaceAdaptive(const double & density,
         const std::vector<std::vector<double> > & planeLocations,
         const std::vector<double>               & fieldLocation,
         const double                      & electricConstant,
         const int                         & faceMethod,
         const double                      & integrationError)
    {
      //
             ...
             std::abs(fieldFromPlanes[2] - fieldOld[2]) > integrationError)
      {
        ...
        //
        for(int iTriangle = 0;
            iTriangle < int(trianglesAllLevels[nLevel].size());
            ++iTriangle)
        {    
          //
    ...
    //  AddTriangleCluster()
    //
    void AddTriangleCluster(const int     quasi,
          const std::vector<double> &node_location,
          const std::vector<int> site_1,
          const std::vector<int> site_2,
          const std::vector<int> site_3,
          const double &electric_constant,
          struct lattice_t &lattice,
          ...
          const int &face_method,
          const double &test_shift,
          const std::vector<double> &face_shift,
          const int &const_index,
          const struct node_list_t &zero_node_list,
          const std::vector<std::vector<std::pair<std::vector<int>,int> > > &node_hash_list,
          std::vector<double> &electric_field,
          double &electric_potential)
    {
      // check if all sites are atomistic, return, contribution included in atomistics
      ...
      latticeC->getSiteCurrentPosition(&location_1[0], 
        &site_1[0], 
        &lattice,
        quasi);
      latticeC->getSiteCurrentPosition(&location_2[0], 
        &site_2[0], 
        &lattice,
        quasi);
      latticeC->getSiteCurrentPosition(&location_3[0], 
        &site_3[0], 
        &lattice,
        quasi);
      
      // check if site is a node, if so find element that point is in
      const unsigned int bucket = getTripletIntegersKey(site_1[0],site_1[1],site_1[2]);

      int node_number;
      const bool site_is_a_node = IsSiteANode(site_1, node_hash_list[bucket], node_number);
      
      // get site point to check
      std::vector<double> point_to_check =
        miscC->vectorScale(MiscFunctions::getInstance()->vectorAddition(location_1,
                 location_2,
                 location_3),1.0/3.0);
      ...
      {   
        ...
        // loop over elements
        for(int i_element = 0; i_element < element_list.number_elements; ++i_element)
        {
          // get element
          const struct element_t *P_element = element_list.elements[i_element];

          // check if point is in element
          if (miscC->checkPointInTetra(P_element->node[0]->position,
      ...
      else
      {
        ...
        const struct element_t *P_element =
          Element::getInstance()->locateSiteElement(&(shape[0]), 
            &l[0], 
            &lattice,
            quasi);
      ...
      const std::vector<double> normal = outwardNormal(triangle_locations,
                   point_to_check);
      ...
      const std::pair<std::vector<double>,double> return_field = 
        fieldFromDensityFaceAdaptive(charge_density,
             triangle_locations,
             node_location,
             electric_constant,
             face_method,
    ...
    //
    void ContributionFromSingleClusterSurface(const int       &quasi,
        const std::vector<double> &node_location,
        const int &face_method,
        const double &integration_error,
        const double &electric_constant,
        const std::vector<int> &face_site,
        const int &const_index,
        const int &iterate_index_1,
        const int &iterate_index_2,
        const double &num_shells,
        const std::vector<std::vector<double> > &element_polarization,
        const std::vector<std::vector<std::vector<int> > > &atomistic_charges,
        const std::vector<double> &face_shift,
        const double &test_shift,
                ...
                double &electric_potential)
    {
        ...
        ++current_site[iterate_index_1])
      {
        for(current_site[iterate_index_2] = 
            ...
            current_site[iterate_index_2] <= face_site[iterate_index_2] + num_shells;
        ...
        {    
          ...
          if(latticeC->isSiteInsideLattice(&lattice, &current_site[0], quasi) == RETURN_FAILURE)
            include_triangle_1 = false;
          if(latticeC->isSiteInsideLattice(&lattice, &site_2[0], quasi) == RETURN_FAILURE)
          {
            include_triangle_1 = false;
            include_triangle_2 = false;
          }
          if(latticeC->isSiteInsideLattice(&lattice, &site_3[0], quasi) == RETURN_FAILURE)
          {
            include_triangle_1 = false;
            include_triangle_2 = false;
          }
          if(latticeC->isSiteInsideLattice(&lattice, &site_4[0], quasi) == RETURN_FAILURE)
            include_triangle_2 = false;

          // add contribution from triangle, potentially
          if(include_triangle_1 == true)
            AddTriangleCluster(quasi,
                   node_location,
                   current_site,
                   site_2,
                   site_3,
    ...
    //
    void AddShellTriangles(const std::vector<std::vector<int> > & coords,
         struct lattice_t & zero_lattice,
         ...
         const int                & zero_quasi)
    {
        ...
        ++current_site[iterate_index_1])
      {
        ...
        {    
          ...
          // check if sites are in lattice
          if(latticeC->isSiteInsideLattice(&zero_lattice, &current_site[0], zero_quasi) 
              == RETURN_FAILURE)
            include_triangle_1 = false;
          if(latticeC->isSiteInsideLattice(&zero_lattice, &site_2[0], zero_quasi) 
              == RETURN_FAILURE)
          {
            include_triangle_1 = false;
            include_triangle_2 = false;
          }
          if(latticeC->isSiteInsideLattice(&zero_lattice, &site_3[0], zero_quasi) 
              == RETURN_FAILURE)
          {
            include_triangle_1 = false;
            include_triangle_2 = false;
          }
          if(latticeC->isSiteInsideLattice(&zero_lattice, &site_4[0], zero_quasi) 
              == RETURN_FAILURE)
            include_triangle_2 = false;
          ...
          if(include_triangle_1 == true)
          {
               ...
               isSiteAtomistic(site_3, atomistic_charges) == false)
            {
              ...
              points.push_back(Point_3(location_1[0], location_1[1], location_1[2]));
              points.push_back(Point_3(location_2[0], location_2[1], location_2[2]));
              points.push_back(Point_3(location_3[0], location_3[1], location_3[2]));
              shell_triangles.push_back(Triangle_3(points[0], points[1], points[2]));
            }
          }
    ...
    // get shell of triangles
    //
    void GetShellOfTriangles(const std::vector<std::vector<int> > & coords,
           struct lattice_t & zero_lattice,
           const std::vector<double> & zero_shift,
           const std::vector<std::vector<std::vector<int> > > & atomistic_charges,
           std::vector<Triangle_3> & shell_triangles,
    ...
    //
    bool NodeInCluster(const std::vector<int> &node_site,
          ...
          std::vector<My_triangle> & integration_triangles)
    {
      ...
      {
        const My_point point = {CGAL::to_double(unique_intersection_points[i_point].x()),
      ...
      if(normal[0] >= normal[1] && normal[0] >= normal[2])
      {
        ...
        for(int i_point = 0; i_point < unique_intersection_points.size(); ++i_point)
          unique_intersection_points_2d.push_back(Point_2(
        ...
        for(int i_point = 0; i_point < unique_intersection_points_2d.size(); ++i_point)
        {
          // loop over all points
          for(int j_point = 0; j_point < intersection_points_double.size(); ++j_point)
          {
            // check if point is within tolerance
            const double y_diff = CGAL::to_double(unique_intersection_points_2d[i_point][0])
              - intersection_points_double[j_point][1];
            const double z_diff = CGAL::to_double(unique_intersection_points_2d[i_point][1])
              - intersection_points_double[j_point][2];
            if(y_diff < 0.001 && y_diff > -0.001 &&
               z_diff < 0.001 && z_diff > -0.001)
            {
      ...
      else if(normal[1] >= normal[0] && normal[1] >= normal[2])
      {
        ...
        for(int i_point = 0; i_point < unique_intersection_points.size(); ++i_point)
          unique_intersection_points_2d.push_back(Point_2(
                      unique_intersection_points[i_point].x(),
                      unique_intersection_points[i_point].z()));
        ...
        for(int i_point = 0; i_point < unique_intersection_points_2d.size(); ++i_point)
        {
          ...
          {
            // check if point is within tolerance
            const double x_diff = CGAL::to_double(unique_intersection_points_2d[i_point][0])
              - intersection_points_double[j_point][0];
            const double z_diff = CGAL::to_double(unique_intersection_points_2d[i_point][1])
              - intersection_points_double[j_point][2];
            if(x_diff < 0.001 && x_diff > -0.001 &&
               z_diff < 0.001 && z_diff > -0.001)
            {
              // insert point into ordered list
              ordered_points.push_back(intersection_points_double[j_point]);
      ...
      {  
        ...
        for(int i_point = 0; i_point < unique_intersection_points_2d.size(); ++i_point)
        {
          ...
          for(int j_point = 0; j_point < intersection_points_double.size(); ++j_point)
          {
            ...
            const double x_diff = CGAL::to_double(unique_intersection_points_2d[i_point][0])
              - intersection_points_double[j_point][0];
            const double y_diff = CGAL::to_double(unique_intersection_points_2d[i_point][1])
              - intersection_points_double[j_point][1];
            if(x_diff < 0.001 && x_diff > -0.001 &&
               y_diff < 0.001 && y_diff > -0.001)
            {
              // insert point into ordered list
              ordered_points.push_back(intersection_points_double[j_point]);
    ...
    //
    void InterpolateValuesAtPoint(cluster_site_data_t   iCSite_data,
             int                                        iQuasi,
             const std::vector<std::vector<double> >    electric_field,
             const std::vector<double>                  electric_potential,
             double                                     charge,
             ...
             double                                     &energy_iC)    
    {
      ...
      // get J
      const double J = miscC->dotProduct(miscC->crossProduct3x3(
          node_difference_1,node_difference_2),node_difference_3);
      ...
      for(int i_dof = 0; i_dof < 4; ++i_dof)
        f_iC[i_dof] = 
            (a1 + b1 * site_state[0] + c1 * site_state[1] + d1 * site_state[2]) * electric_field[node_0][i_dof] 
          + (a2 + b2 * site_state[0] + c2 * site_state[1] + d2 * site_state[2]) * electric_field[node_1][i_dof] 
          + (a3 + b3 * site_state[0] + c3 * site_state[1] + d3 * site_state[2]) * electric_field[node_2][i_dof] 
          + (a4 + b4 * site_state[0] + c4 * site_state[1] + d4 * site_state[2]) * electric_field[node_3][i_dof];

      for(int i_dof = 0; i_dof < 4; ++i_dof)
        f_iC[i_dof] *= charge;

      // get energy
      energy_iC = 0.5 * charge * (
          (a1 + b1 * site_state[0] + c1 * site_state[1] + d1 * site_state[2]) * electric_potential[node_0] 
        + (a2 + b2 * site_state[0] + c2 * site_state[1] + d2 * site_state[2]) * electric_potential[node_1] 
        + (a3 + b3 * site_state[0] + c3 * site_state[1] + d3 * site_state[2]) * electric_potential[node_2] 
        + (a4 + b4 * site_state[0] + c4 * site_state[1] + d4 * site_state[2]) * electric_potential[node_3]
        );
  ...
  namespace
  {
    ...
    void * computeFacesWorker(void * arg)
    {
      ...
      struct element_list_t & elementList = 
        *(computeFaces_argument->elementList);
      ...
      for (int iElem = number_start; iElem <= number_end; ++iElem)
      {
        ...
        for(int iFace = 0; iFace < 4; ++iFace)
        {
          ...
          // get bucket
          //
          unsigned int bucket = getTripletIntegersKey(elementFaces[iFace][0],
                  elementFaces[iFace][1],
    ...
    void * computePolarizationSitesWorker(void * arg)
    {
      ...
      //
      struct computePolarizationSites_argument_t * computePolarizationSites_argument = 
      ...
      struct lattice_t & lattice = 
        *(computePolarizationSites_argument->lattice);
      ...
      for (int iElem = number_start; iElem <= number_end; ++iElem)
      {
        ...
        for (int iDof = 0; iDof < 3; ++iDof)
        {
          ...
          for (int iNode = 0; iNode < 4; ++iNode)
          {
            ...
            //
            center[iDof] += elementList.elements[iElem]->node[iNode]->initial_position[iDof];
          }
    ...
    void * computeAtomisticElementsWorker(void * arg)
    {
      ...
      //
      struct computeAtomisticElements_argument_t * computeAtomisticElements_argument = 
      ...
      for (int iElem = number_start; iElem <= number_end; ++iElem)
      {
        ...
        double elementVolume = 
          tetrahedronVolume(elementList.elements[iElem]->node[0]->initial_position,
    ...
    void * computeAtomisticElementLocationsWorker(void * arg)
    {
      ...
      //
      struct computeAtomisticElementLocations_argument_t * computeAtomisticElementLocations_argument = 
        static_cast<struct computeAtomisticElementLocations_argument_t *>(arg);
    ...
    //
    void * 
    computeAtomisticNodesWorker(void * arg)
    {
      //
      ...
      struct node_list_t & nodeList = 
        *(computeAtomisticNodes_argument->nodeList);
      std::vector<std::pair<int,std::vector<int> > > & atomisticNodes =
      ...
      for (int iNode = number_start; iNode <= number_end; ++iNode)
      {
        ...
        for(int iElem = 0; iElem < numElem; ++iElem)
          elemNumbers.push_back(nodeList.nodes[iNode]->element_list.elements[iElem]->number);
      ...
      { 
        ...
        if(sortVariable == my_id)
        {
          ...
          // insert nodes on end
          //
          atomisticNodes.insert(atomisticNodes.end(),
              localAtomisticNodes.begin(),
    ...
    void * computeAtomisticChargesWorker(void * arg)
    {
      ...
      struct lattice_t & lattice = 
        *(computeAtomisticCharges_argument->lattice);
            struct element_list_t & elementList = 
          ...
          if (atomisticElements[iElem] == 0)
          {
            ...
            for(int iNode = 0; iNode < 4; ++iNode)
            {
              ...
              for(int iSite = 0; iSite < int(addSites.size()); ++iSite)
              {
                ...
                // get bucket
                //
                int bucket = getTripletIntegersKey(addSites[iSite][0],
                           addSites[iSite][1],
                           addSites[iSite][2]);
                ...
                // insert into bucket
                //
                insertChargeIntoBucket(addSites[iSite],
                           atomisticCharges[bucket]);
  ...
  namespace
  {
    ...
    //
    void * 
    computePolarizationLocationsWorker(void * arg)
    {
      //
      // cast back to get computeAtomisticNodes_argument_t
      //
      struct computePolarizationLocations_argument_t * computePolarizationLocations_argument = 
      ...
      struct lattice_t & lattice = 
        *(computePolarizationLocations_argument->lattice);
      ...
      while(whileWorker)
      {
        ...
        for (int iElem = number_start; iElem < number_end; ++iElem)
        {
          ...
          if (atomisticElements[iElem] == 1)
          {
            ...
            if(Lattice::getInstance()->isSiteInsideLattice(&lattice, &site[0], iQuasi) == RETURN_FAILURE)
            {
            ...
            Lattice::getInstance()->getSiteCurrentPosition(location, 
              site, 
              &lattice,
              iQuasi);
            ...
            //
            elementPolarizationLocations[iElem].push_back(std::make_pair(tempLocation,
              charge));
          }
        }
      }
    ...
    //
    void * 
    computeElementPolarizationWorker(void * arg)
    {
      //
      // cast back to get computeAtomisticNodes_argument_t
      //
      struct computeElementPolarization_argument_t * computeElementPolarization_argument = 
      ...
      double initialUnitCellVolume = computeElementPolarization_argument->initialUnitCellVolume;
      struct element_list_t & elementList = 
      ...
      while(whileWorker)
      {
        ...
        for (int iElem = number_start; iElem < number_end; ++iElem)
        {
          ...
          if(atomisticElements[iElem] == 1)
          {
            ...
            //
            double changeInVolume = 
              currentElementVolume / initialElementVolume;
            ...
            for(int iAtom = 0; iAtom < int(elementPolarizationLocations[iElem].size()); ++iAtom)
              for(int iDof = 0; iDof < 3; ++iDof)
                polarization[iDof] 
                  += (elementPolarizationLocations[iElem][iAtom].first[iDof] 
                    - elementPolarizationLocations[iElem][0].first[iDof]) * elementPolarizationLocations[iElem][iAtom].second;
    ...
    void * computeChargeDensitiesWorker(void * arg)
    {
      ...
      struct node_list_t & nodeList = 
        *(computeChargeDensities_argument->nodeList);
      std::vector<std::vector<double> > & elementPolarization = 
        *(computeChargeDensities_argument->elementPolarization);
      std::vector<std::vector<std::pair<double,std::vector<std::vector<double> > > > > & globalChargeDensity = 
        *(computeChargeDensities_argument->chargeDensity);
      struct lattice_t & lattice = 
        *(computeChargeDensities_argument->lattice);
      std::vector<std::pair<int,double> > & electroBoundaries = 
      ...
      while(whileWorker)
      {
        ...
        for (int iBucket = number_start; iBucket < number_end; ++iBucket)
        {
          ...
          for (int iFace = 0; iFace < int(faces[iBucket].size()); ++iFace)
          {
            ...
            std::vector<double> normal = outwardNormal(nodeLocations,
                         fourthNodeLocation);
            ...
            // if there is no second element, then face has a void or edge on the other side
            // assume polar2 = 0, can also assume polar2 = polar1 for no contribution, see commented lines below
            if(element2 != -1)
            {
              ...
              for (int iDof = 0; iDof < 3; ++iDof)
                nodeLocations[4][iDof] = nodeList.nodes[fourthNode2]->position[iDof];
              ...
              globalChargeDensity[iBucket].push_back(std::make_pair(chargeDensity,
                          nodeLocations));
    ...
    //
    void * 
    computeAtomisticChargeStateWorker(void * arg)
    {
      //
      // cast back to get computeAtomisticChargeLocations_argument_t
      //
      struct computeAtomisticChargeState_argument_t * computeAtomisticChargeState_argument = 
      ...
      struct lattice_t & lattice = 
        *(computeAtomisticChargeState_argument->lattice);
      ...
      while(whileWorker)
      {
        ...
        for (int iBucket = number_start; iBucket < number_end; ++iBucket)
        {
          ...
          // loop over charges
          //
          for (int iCharge = 0; iCharge < int(atomisticCharges[iBucket].size()); ++iCharge)
          {
            //
            // temporary location
            //
            ...
            // get site location
            //
            //if(quasicontinuum::is_site_inside_lattice(&lattice,site) == RETURN_FAILURE)
            if(Void::getInstance()->findSiteInVoidCache(atomisticCharges[iBucket][iCharge],
              iQuasi) == true)
              continue;
            
            Lattice::getInstance()->getSiteCurrentState(state, 
              site, 
              &lattice,
              iQuasi);
    ...
    void * computeFieldFromAtomisticChargesWorker(void * arg)
    {
      ...
      struct computeFieldFromAtomisticCharges_argument_t * computeFieldFromAtomisticCharges_argument = 
        static_cast<struct computeFieldFromAtomisticCharges_argument_t *>(arg);

      // get handles
      const struct node_list_t & nodeList = *(computeFieldFromAtomisticCharges_argument->nodeList);
      std::vector<std::vector<double> > & electricField = 
      ...
      const std::vector<double> & iShift = *(computeFieldFromAtomisticCharges_argument->iShift);
      ...
      while(whileWorker)
      {
        ...
        for (int i_node = number_start; i_node < number_end; ++i_node)
        {
          ...
          for(int i_dof = 0; i_dof < 3; ++i_dof)
            node_state[i_dof] = nodeList.nodes[i_node]->position[i_dof] + iShift[i_dof];

          node_state[3] = nodeList.nodes[i_node]->frequency;

          // get field from atomistic charges
          for(int j_quasi = 0; j_quasi < int(atomisticChargeState.size()); ++j_quasi)
          {
            //
            //  get core flag
            //
            ...
            for(int j_charge = 0;
              j_charge < atomisticChargeState[j_quasi].size();
              ++j_charge)
            {
              AddAtomicToFieldAndPotential(node_state,
                  iQuasi,
                  atomisticChargeState[j_quasi][j_charge],
                  j_quasi,
                  electricConstant,
                  charge,
                  electricField[i_node],
                  electricPotential[i_node],
                  coreData);
            }
    ...
    void * computeClusterChargesWorker(void * arg)
    {
      ...
      struct lattice_t & j_lattice =
        *(computeClusterCharges_argument->j_lattice);
      const std::vector<double> & i_shift = *(computeClusterCharges_argument->i_shift);
      const std::vector<double> & j_shift = *(computeClusterCharges_argument->j_shift);
              ...
              ++x_site)
          {
                ...
                ++y_site)
            {
                  ...
                  ++z_site)
              {
                ...
                if(Lattice::getInstance()->isSiteInsideLattice(&j_lattice, &site[0], j_quasi) == RETURN_FAILURE)
                  continue;
                ...
                Lattice::getInstance()->getSiteCurrentState(&jSiteState[0], 
                  &site[0], 
                  &j_lattice,
                  j_quasi);
    ...
    void * ComputeFieldFromExternalChargesWorker(void * arg)
    {
      ...
      struct compute_field_from_external_charges_argument_t * compute_field_from_external_charges_argument = 
        static_cast<struct compute_field_from_external_charges_argument_t *>(arg);

      // get handles
      const struct node_list_t & node_list = *(compute_field_from_external_charges_argument->node_list);
      std::vector<std::vector<double> > & electric_field = 
      ...
      const int & iQuasi = 
        *(compute_field_from_external_charges_argument->iQuasi);
      int & iCoreFlag = 
        *(compute_field_from_external_charges_argument->iCoreFlag);
      ...
      while(whileWorker)
      {
        ...
        for (int i_node = number_start; i_node < number_end; ++i_node)
        {
          // get global node location
          for(int i_dof = 0; i_dof < 3; ++i_dof)
            node_state[i_dof] = node_list.nodes[i_node]->position[i_dof] + iShift[i_dof];
          ...
          {            
            ...
            AddAtomicToFieldAndPotentialExternal(node_state,
                 iQuasi,
                 external_charges[j_charge].second,
                 electric_constant,
                 external_charges[j_charge].first,
                 electric_field[i_node],
                 electric_potential[i_node],
                 iCoreFlag);
    ...
    void * computeClusterChargeSurfaceWorker(void *arg)
    {
      ...
      struct computeClusterChargeSurface_argument_t *computeClusterChargeSurface_argument = 
      ...
      int  zero_quasi = 
        *(computeClusterChargeSurface_argument->zero_quasi);        
      ...
      while(whileWorker)
      {
        ...
        for (int i_node = number_start; i_node < number_end; ++i_node)
        {
          ...
          // set base site index
          const std::vector<int> base_site_index = {i_node_list.nodes[i_node]->l[0],
                      i_node_list.nodes[i_node]->l[1],
                      i_node_list.nodes[i_node]->l[2]};
          ...
          if(face_site[0] != zero_lattice.l_start[0])
            ContributionFromSingleClusterSurface(zero_quasi,
                   node_location,
                   face_method,
                   integration_error,
                   electric_constant,
    ...
    void * computeChargedFaceWorker(void *arg)
    {
      ...
      // get handles
      const struct node_list_t & i_node_list = *(computeChargedFace_argument->i_node_list);
      std::vector<std::vector<double> > & electric_field =
        *(computeChargedFace_argument->electricField);
      std::vector<double> & electric_potential =
        *(computeChargedFace_argument->electricPotential);
      const double & electric_constant = *(computeChargedFace_argument->electricConstant);
      const std::vector<double> & i_shift = *(computeChargedFace_argument->i_shift);
      const std::vector<double> & zero_shift = *(computeChargedFace_argument->zero_shift);
      const int & num_shells = *(computeChargedFace_argument->num_shells);
      const int & face_method = *(computeChargedFace_argument->face_method);
      const double & integration_error = *(computeChargedFace_argument->integration_error);
      const struct node_list_t & zero_node_list =
      ...
      const std::vector<std::vector<std::pair<double,std::vector<std::vector<double> > > > > & charge_density = *(computeChargedFace_argument->charge_density);
      ...
      while(whileWorker)
      {
        ...
        for (int i_node = number_start; i_node < number_end; ++i_node)
        {
          ...
          for(int i_dof = 0; i_dof < 3; ++i_dof)
            node_site[i_dof] =
              i_node_list.nodes[i_node]->l[i_dof];
          ...
          const std::vector<std::vector<int> > coords = {x_coords, y_coords, z_coords};
          ...
          {
            for(int i_face = 0; i_face < charge_density[i_bucket].size(); ++i_face)
            {
              // get face node sites
              ...
              for(int i_node = 0; i_node < 3; ++i_node)
              {
                ...
                for(int i_dof = 0; i_dof < 3; ++i_dof)
                  temp_site.push_back(zero_node_list.nodes[face_node_numbers[i_node]]->l[i_dof]);
              ...
              for(int i_node = 0; i_node < 3; ++i_node)
                face_node_in_cluster.push_back(NodeInCluster(node_site,
                         num_shells,
                         face_node_sites[i_node]));
              ...
              Point_3(zero_node_list.nodes[face_node_numbers[i_node]]->position[0] + zero_shift[0],
                zero_node_list.nodes[face_node_numbers[i_node]]->position[1] + zero_shift[1],
                zero_node_list.nodes[face_node_numbers[i_node]]->position[2] + zero_shift[2]));
              ...
              for(int i_node = 0; i_node < 3; ++i_node)
              {
                ...
                for(int i_dof = 0; i_dof < 3; ++i_dof)
                 ...
                 zero_node_list.nodes[face_node_numbers[i_node]]->position[i_dof] + zero_shift[i_dof]);
              ...
              const std::pair<std::vector<double>,double> pos_return_field = 
                fieldFromDensityFaceAdaptive(charge_density[i_bucket][i_face].first,
                     face_node_locations_double,
                     node_location,
                     electric_constant,
                     face_method,
                     integration_error);
              ...
              for(int i_triangle = 0; i_triangle < shell_triangles.size(); ++i_triangle)
              {
                // check intersection
                CGAL::Object result = intersection(shell_triangles[i_triangle],
                           face_node_triangle);
                ...
                {
                  for(int i_point = 0; i_point < return_vector_points.size(); ++i_point)
              ...
              for(int i_point = 0; i_point < intersection_points.size(); ++i_point)
              {
                bool insert_point = true;
                for(int j_point = 0;
                    j_point < unique_intersection_points.size(); ++j_point)
                  if(CGAL::squared_distance(intersection_points[i_point],
                          unique_intersection_points[j_point]) < 0.1)
                    insert_point = false;
                
                if(insert_point == true)
                  unique_intersection_points.push_back(intersection_points[i_point]);
              }
              ...
              for(int i_node = 0; i_node < 3; ++i_node)
              {
                if(face_node_in_cluster[i_node] == true)
                {
                  ...
                  for(int j_point = 0;
                      j_point < unique_intersection_points.size(); ++j_point)
                    if(CGAL::squared_distance(face_node_locations_point[i_node],
                            unique_intersection_points[j_point]) < 0.1)
                      insert_node = false;
                  
                  if(insert_node == true)
                    unique_intersection_points.push_back(face_node_locations_point[i_node]);
                }
              }
  ...
  void
  Electrostatics::destroyInstance()
  {
  ...
  //  insertInputData()
  //
  void Electrostatics::insertInputData(const std::vector<double> charges,
  ...
  void Electrostatics::computeGeometries(void)
  {
    ...
    //
    //  if all elements associated to node are atomistic then declare node as atomistics
    //
  ...
  void Electrostatics::computeFaces(struct element_list_t * P_element_list)
  {
    ...
    //
    thread_monitor(computeFacesWorker, static_cast<void *>(&computeFaces_argument),
    ...
    // std::cout<<faceNum<<" "<<iBucket<<" "<<iFace<<" "<<d_faces[iBucket][iFace][0]<<" "<<d_faces[iBucket][iFace][1]<<" "<<d_faces[iBucket][iFace][2]<<std::endl;
    // faceNum++;
    //     }
    //   }
  ...
  //
  void 
  Electrostatics::computePolarizationSites(struct element_list_t *P_element_list,
  ...
  //
  void 
  Electrostatics::computeAtomisticElements(struct element_list_t *P_element_list)
  {
    ...
    computeAtomisticElements_argument.atomisticElementSize = &(d_atomisticElementSize);
  ...
  //
  void 
  Electrostatics::computeAtomisticElementLocations(struct element_list_t *P_element_list)
  {
    //
    ...
    // set d_atomisticElements to correct size
    //
    if (P_element_list->number_elements != int(d_atomisticElementLocations.size()))
      d_atomisticElementLocations.resize(P_element_list->number_elements);

    //
    // instantiate computeAtomisticElements_argument_t
    //
    computeAtomisticElementLocations_argument_t computeAtomisticElementLocations_argument;
    
    //
    // find atomistic element list
    //
    computeAtomisticElementLocations_argument.atomisticElements = &(d_atomisticElements);
    computeAtomisticElementLocations_argument.atomisticElementLocations = &(d_atomisticElementLocations);
    computeAtomisticElementLocations_argument.elementList = P_element_list;
  
    //
    // distribute work to threads
    //
    thread_monitor(computeAtomisticElementLocationsWorker,
  ...
  //
  void 
  Electrostatics::computeAtomisticNodes(struct node_list_t *P_node_list)
  {
    //
  ...
  //
  void 
  Electrostatics::computeAtomisticCharges(struct lattice_t *P_lattice,
                  struct element_list_t *P_element_list)
  {
    //
  ...
  void Electrostatics::ComputeBaseNodeHashList(struct node_list_t *P_node_list)
  {
    ...
    for(int i_node = 0; i_node < P_node_list->number_nodes; ++i_node)
    {
      ...
      // get bucket
      const unsigned int i_bucket = getTripletIntegersKey(site[0], site[1], site[2]);
  ...
  //
  void 
  Electrostatics::clearGeometries(void)
  {
  ...
  //
  void 
  Electrostatics::computeElectricField(const bool compute_in_reference)
  {
    ...
    struct lattice_t lattice = 
      quasicontinua->getQuasi(0).getLattice();
  ...
  //
  void 
  Electrostatics::computePolarization(struct element_list_t *P_element_list)
  {
    //
  ...
  //
  void 
  Electrostatics::computeChargeDensities(struct node_list_t *P_node_list,
                 struct lattice_t   *P_lattice)
  {
    //
    ...
    computeChargeDensities_argument.elementPolarization = &(d_elementPolarization);
  ...
  //
  void 
  Electrostatics::computeAtomisticChargeState()
  {
    //
    // get Quasicontinua
    //
    Quasicontinua * quasicontinua = 
      Quasicontinua::getInstance();
                  ...
                  const bool &compute_in_reference)
  {
    ...
    {
      // get number of nodes and node list
      struct node_list_t & node_list = quasicontinua->getQuasi(i_quasi).getNodeList().node_list;
      const int num_nodes = node_list.number_nodes;
      ...
      computeFieldFromAtomisticCharges_argument_t computeFieldFromAtomisticCharges_argument;
      ...
      if(compute_in_reference == true)
      {
        computeFieldFromAtomisticCharges_argument.electricField = &(d_electricFieldReference[i_quasi]);
        computeFieldFromAtomisticCharges_argument.electricPotential = &(d_electricPotentialReference[i_quasi]);
      }
      else
      {
        computeFieldFromAtomisticCharges_argument.electricField = &(d_electricField[i_quasi]);
        computeFieldFromAtomisticCharges_argument.electricPotential = &(d_electricPotential[i_quasi]);
      }
      computeFieldFromAtomisticCharges_argument.fixedCharge = &(d_fixedCharge);
      computeFieldFromAtomisticCharges_argument.electricConstant = &(d_electricConstant);
      computeFieldFromAtomisticCharges_argument.atomisticChargeState = &(d_atomisticChargeState);
      computeFieldFromAtomisticCharges_argument.iShift = &shift;
      computeFieldFromAtomisticCharges_argument.iQuasi = &i_quasi;

      // distribute work to threads
      thread_monitor(computeFieldFromAtomisticChargesWorker,
                  ...
                  const bool &compute_in_reference)
  {
    ...
    for(int i_quasi = 0; i_quasi < num_quasi; ++i_quasi)
    {
      ...
      struct node_list_t & node_list = quasicontinua->getQuasi(i_quasi).getNodeList().node_list;
      std::vector<double> i_shift = Quasicontinua::getInstance()->getShift(i_quasi);
      ...
      for(int j_quasi = 0; j_quasi < num_quasi; ++j_quasi)
      {
        ...
        std::vector<double> j_shift = Quasicontinua::getInstance()->getShift(j_quasi);
        ...
        {
          computeClusterCharges_argument.electricField = &(d_electricField[i_quasi]);
          computeClusterCharges_argument.electricPotential =
                  ...
                  const bool &compute_in_reference)
  {
    ...
    for(int i_quasi = 0; i_quasi < num_quasi; ++i_quasi)
    {
      ...
      compute_field_from_external_charges_argument_t compute_field_from_external_charges_argument;
    
      // set data structures
      compute_field_from_external_charges_argument.node_list = &node_list;
      compute_field_from_external_charges_argument.electric_field = &(d_electricField[i_quasi]);
      compute_field_from_external_charges_argument.electric_potential = &(d_electricPotential[i_quasi]);
      compute_field_from_external_charges_argument.electric_constant = &(d_electricConstant);
      compute_field_from_external_charges_argument.external_charges = &(d_externalPointCharges);
      ...
      // distribute work to threads
      thread_monitor(ComputeFieldFromExternalChargesWorker,
                  ...
                  const bool &compute_in_reference)
  {
    ...
    for(int i_quasi = 0; i_quasi < num_quasi; ++i_quasi)
    {
      ...
      {
        computeClusterChargeSurface_argument.electricField = &(d_electricField[i_quasi]);
        computeClusterChargeSurface_argument.electricPotential =
          &(d_electricPotential[i_quasi]);
      }

      computeClusterChargeSurface_argument.electricConstant = &(d_electricConstant);
      computeClusterChargeSurface_argument.atomisticCharges = &(d_atomisticCharges);
      computeClusterChargeSurface_argument.element_polarization = &(d_elementPolarization);
      ...
      computeClusterChargeSurface_argument.integration_error = &d_integration_error;
      computeClusterChargeSurface_argument.zero_node_list = &zero_node_list;
      computeClusterChargeSurface_argument.base_node_hash_list = &d_base_node_hash_list;
  ...
  Electrostatics::computeElementPolarizationLocations()
  {
    ...
    for(int iQuasi = 0; iQuasi < numQuasi; ++iQuasi)
    {
      ...
      //
      computePolarizationLocations_argument_t computePolarizationLocations_argument;
  ...
  //
  void 
  Electrostatics::computeElementPolarization(struct element_list_t *P_element_list)
  {
    //
    // get Quasicontinua
    //
    ...
    double unitCellVolume = 
      (lattice.a1[1] * lattice.a2[2] - lattice.a1[2] * lattice.a2[1]) * lattice.a3[0] + 
      (lattice.a1[2] * lattice.a2[0] - lattice.a1[0] * lattice.a2[2]) * lattice.a3[1] +
      (lattice.a1[0] * lattice.a2[1] - lattice.a1[1] * lattice.a2[0]) * lattice.a3[2];
  ...
  //
  void 
  Electrostatics::computeElectricFieldReference()
  {
    // get quasi holder
    Quasicontinua * quasicontinua = 
      Quasicontinua::getInstance();
    ...
    for(int iQuasi = 1; iQuasi < numQuasi; ++iQuasi)
    {
      ...
      for(int iNode = 0; iNode < numberNodes; ++iNode)
      {
        ...
        for(int iDof = 0; iDof < 3; ++iDof)
          nodePositions[iQuasi][iNode].push_back(quasiNodeList.nodes[iNode]->position[iDof]);

        // set current position to initial position
        for(int iDof = 0; iDof < 3; ++iDof)
          quasiNodeList.nodes[iNode]->position[iDof] = quasiNodeList.nodes[iNode]->initial_position[iDof];
      } // end loop over nodes
    } // end loop over quasi

    // save external forces and zero out current
    const std::vector<std::pair<double,std::vector<double> > > saved_external_point_charges{d_externalPointCharges};
    d_externalPointCharges.clear();
    ...
    for(int i_charge = 0; i_charge < saved_external_point_charges.size(); ++i_charge)
    ...
    for(int iQuasi = 1; iQuasi < numQuasi; ++iQuasi)
    {
      ...
      for(int iNode = 0; iNode < numberNodes; ++iNode)
      {
        // save current position
        for(int iDof = 0; iDof < 3; ++iDof)
         quasiNodeList.nodes[iNode]->position[iDof] = nodePositions[iQuasi][iNode][iDof];
      } // end loop over nodes
    } // end loop over quasi
  ...
  //  
  void Electrostatics::computeForceOnNodeDueToSite(int iQuasi,
      ...
      bool                            compute_in_reference)  
  {
    ...
    else
    {
      InterpolateValuesAtPoint(iCSite_data,
             iQuasi,
             d_electricField[iQuasi],
             d_electricPotential[iQuasi],
             d_fixedCharge[iQuasi],
             f_iC,
             energy_iC);
    }
    ...
    struct lattice_t iQuasi_lattice = quasicontinua->getQuasi(iQuasi).getLattice();

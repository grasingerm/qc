...
#if !defined(lint)
static const char rcsid[]="$Id: site_element_map_cache.c,v 1.8 2002/03/07 23:53:09 knap Exp $";
#endif /* !lint */
...
*/
static pthread_rwlock_t rwlock = PTHREAD_RWLOCK_INITIALIZER; //FIXME: May not work on Sun
...
**/
/**
#define hash_function(l) ((((l)[0] << 1)+((l)[1] << 4)+((l)[2] << 8 ))&MODULO_MASK)
**/
...
#define hash_function(l) (((l)[0] & BITS_L1_MASK) | ((l[1]) & BITS_L2_MASK) | ((l)[2] & BITS_L3_MASK)) 
*/
...
static void
hash_table_init(const int iQuasi)
{
  /**
  ...
  if (iQuasi >= hash_table_allocated){
    ...
    if (iQuasi==0){
      ...
      hash_table = realloc(hash_table,
			   (iQuasi+1) * sizeof(struct hash_table_t));
 ...
 /**
  ...
  for(j_Quasi = 0; j_Quasi < (iQuasi + 1); j_Quasi++){
    ...
    /* if (pthread_mutex_init(&(hash_table.buckets[i_bucket].lock), NULL)) { */
      ...
      if (rwlock_init(&(hash_table[j_Quasi].buckets[j_bucket].lock), USYNC_THREAD, 
		    NULL)) {
	ERROR("rwlock_init()");
#else
#ifdef HAVE_PTHREAD_RWLOCK_INIT
      if (pthread_rwlock_init(&(hash_table[j_Quasi].buckets[j_bucket].lock), NULL)) { 
...
static void
add_data_to_bucket(struct hash_bucket_t *P_bucket,
		   const struct datum_t *P_datum)
{
  ...
  if (P_bucket->alloc_space == 0) {
    ...
    if ((P_bucket->data = 
	 realloc(P_bucket->data, sizeof(struct datum_t)*
		 P_bucket->alloc_space)) == NULL) {
      ERROR("realloc()");
      exit(EXIT_FAILURE);
    }
 ...
 */
static void
hash_table_clear_locks(const int iQuasi)
{
  /**
   * define bucket
   */
...
#else
#ifdef HAVE_PTHREAD_RWLOCK_INIT
	if (pthread_rwlock_destroy(&(hash_table[j_Quasi].buckets[j_bucket].lock))) { 
        ERROR("pthread_mutex_init()");
#else
#error No pthread_rwlock_init() found
#endif /* HAVE_PTHREAD_RWLOCK_INIT */
 ...
 */
void
check_hash_table_allocation(const int iQuasi)
{
        ...
        const int               iQuasi)
{
  ...
  datum.shape[0] = computeShapeFunction(P_node_0->initial_position, 
				     P_node_1->initial_position, 
				     P_node_2->initial_position, 
				     P_node_3->initial_position, 
				     X);

  datum.shape[1] = computeShapeFunction(P_node_1->initial_position, 
				     P_node_0->initial_position, 
				     P_node_2->initial_position, 
				     P_node_3->initial_position, 
				     X);
  
  datum.shape[2] = computeShapeFunction(P_node_2->initial_position, 
				     P_node_1->initial_position, 
				     P_node_0->initial_position, 
				     P_node_3->initial_position, 
				     X);

  datum.shape[3] = computeShapeFunction(P_node_3->initial_position, 
				     P_node_1->initial_position, 
				     P_node_2->initial_position, 
				     P_node_0->initial_position, 
				     X);
	      ...
	      const void *d1)
{
  ...
  if ((l0[0] == l1[0]) && (l0[1] == l1[1]) && (l0[2] == l1[2])) return(0); 
...
hash_table_search_element_unlocked(const int l[3], const int iQuasi)
{
  ...
  P_datum = lfind(&datum, &(hash_table[iQuasi].buckets[hash_table_index].data[0]),
		  (size_t *) &(n_data), sizeof(struct datum_t), compare_datum);

  if (P_datum == NULL) return(NULL);
...
#error no pthread_rwlock_rdlock() found.
  ...
  n_data = hash_table[iQuasi].buckets[hash_table_index].n_data;
  P_datum = lfind(&datum, 
      &(hash_table[iQuasi].buckets[hash_table_index].data[0]),
      (size_t *) &(n_data), 
      sizeof(struct datum_t), 
      compare_datum);
...
void
hash_table_clean(const int iQuasi)
{
